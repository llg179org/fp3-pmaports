...
Added 2026-08-28, the day the search closed on the modem. The full account is in
[`findings-log.md`](findings-log.md):

| claim | what it turned out to be |
|---|---|
| "half the gap has no named owner, and it is that pmOS never suspends" | **withdrawn the same afternoon.** The first half stands — `suspend_stats/success = 0`, `IdleAction=ignore`, `sleep-inactive-battery-type='nothing'`, so pmOS has never suspended and nothing asks it to. The attribution does not: `2026-08-24_ut-coulomb-and-sleep-attempt.txt`, in the repository four days already, records the **oracle** managing 2 completed suspends out of 120 attempts with the full recipe applied. Neither system sleeps, so sleep is headroom below *both*, not the difference between them |
| "the oracle idles at ~30 mA" | the low outlier of four. Its `cc_soc` windows read 32.2 / 54.6 / 61.0 / 64.3 mA; the 32.2 is the earliest, the only one on a half-empty pack, and the only one whose own write-up warns the pack was still relaxing. ☠️ **The flattering outlier is the one that travelled** — it became the headline because it was the most striking, not the most defensible, and every later comparison inherited it. On the surviving three the gap is ~40 mA, **1.67×**, not 2× |
| "the two systems do not run the same modem firmware" | ☠️☠️ **dead, and it had been the last live candidate.** `modem_a`, `modem_b` *and* our rootfs copy all carry `QC_IMAGE_VERSION_STRING=MPSS.TA.3.1.C1-425464` and `GEN_PACK-1.356774.1.425464.1`. The `325768` came from the partition's `verinfo/ver_info.txt`, package metadata whose `"modem"` field mirrors `Meta_Build_ID` — and our own image embeds that string too, so a grep confirms the difference from either side. **A metadata file on one side read against a binary's own version string on the other.** The rule that catches it: ask the same question of both sides with the same instrument |
| "the oracle's 6.3 % MPSS duty might be a `Powered=false` modem" | raised as a real hole — the 565 s window records no radio state and the same session ran both ofono modems off later that day — and **closed from our own side without a slot switch**: `--set-power-state-low` puts the MPSS core at **0.0 %**, 186 of 186 samples, and 6.3 % is not 0 % |
| "`mmcli --disable` and `--set-power-state-low` are the same lever" | they are two layers. `--disable` leaves the duty at 34–36 %; `--set-power-state-low` takes it to zero. "Disabled" stops the radio's *use*, "low" powers the *core* down — and a null result from the first was nearly read as a null result about the modem |
| "A vs A′ drift means the radio-low A/B was too noisy to read" | the 22-point drift was **signal, not noise**: `--set-power-state-on` restores power but does not re-enable the modem, so A was registered+attached (51.6 %) and A′ was enabled-but-unregistered (29.3 %). Three states, not one baseline with spread. ☠️ The state command read `power state`, which cannot tell those apart — **a knob's witness must watch the variable the knob changes, not one next to it** |
| "`voltage_min_design` is ignored, so a low-battery cutoff is missing" | there is no cutoff in `qcom_smbx.c` and there should not be — it does not even export `VOLTAGE_MIN_DESIGN`, and on mainline the shutdown is UPower's policy driven by `capacity`. The 2.864 V run happened because the percentage never fell below 35 %. One defect, not two |
