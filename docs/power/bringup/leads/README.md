...
| [`modem-carrier-config.md`](modem-carrier-config.md) | 🔬 **hypothesis, first read not yet taken.** Elimination is exhausted and the network is no longer a variable — the oracle's windows were on the *expensive* cell too, at 5.4-8.1 % against our 48.9-52.7 % on that same cell. What is left has to be something the modem is told once and we never tell it, and there is one such layer the host owns: the carrier configuration (PDC/MBN). ☠️ Not `modem.mbn`, the firmware image, which is identical on both |
