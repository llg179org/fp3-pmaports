...
| [`audio-off-leg.sh`](audio-off-leg.sh) | what does the **audio stack** cost in mA — a boot with the modules never loaded against a boot with them loaded. Decides whether the audio series has to wait for the power work. ☠️ Cannot run inside one boot, so it reads the **floor (p10)**, which has held at 53-54 mA across every knob regardless of duty, and never the median, which carries the modem's per-boot offset. ☠️ The leg's witness is the **LPASS going down**, not the module list — `measure` refuses to label a B leg whose ADSP stayed awake. ☠️ `install <mod> /bin/false`, not `blacklist`; and the NGD module is `slim_qcom_ngd_ctrl`, not `qcom_ngd_ctrl` |
