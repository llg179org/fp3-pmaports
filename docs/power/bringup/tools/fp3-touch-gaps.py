...
# ☠️ BTN_TOUCH IS SINGLE-TOUCH EMULATION and undercounts. It goes to 1 when the
# FIRST finger lands and to 0 when the LAST one lifts, so a second contact
# arriving while the first is down produces no transition at all. Measured
# 2026-09-06: the operator tapped left and right with index and middle finger,
# the compositor delivered 864 taps to a client and this file recorded 784
# presses - the client appearing to receive MORE than the kernel sent, which is
# impossible and is what exposed the flaw. Had the error gone the other way it
# would have been reported as taps lost in the compositor.
#
# ABS_MT_TRACKING_ID counts CONTACTS: a value >= 0 opens a new one, -1 closes
# it. That is the number comparable with what a client receives.
...
RING_SECONDS = 8.0
# ☠️ The gate, built into the instrument, AND WHERE v6 PUT IT WRONG.
#
# v6 gated on the LIFT - the interval with no finger on the panel - and
# demanded interrupts > 0 there. That is not a known-positive, it is a known
# NEGATIVE: no finger means no interrupts, so 17 of its first 30 gate lines
# read "+0" and the gate correctly refused the instrument. The measurement it
# validated against was the wrong one.
#
# The known-positive is the HOLD: while a finger is down and frames are being
# delivered, the counter MUST rise. So the gate now fires on RELEASE and reads
# the hold. If a hold shows +0 interrupts, the sampler or the
# /proc/interrupts parser is broken and nothing this tool says may be quoted.
VALIDATE_FIRST = 30

# ☠️ THE BOUNDARY, and the artefact it caused in v7.
#
# /proc/interrupts counts the HARD interrupt, which fires before the threaded
# handler runs and long before BTN_TOUCH reaches userspace. A window ending at
# the press's evdev timestamp therefore contains THE PRESS'S OWN INTERRUPT, and
# v7 read that as "the chip reported and no press came". Measured 2026-09-06:
# quiet lifts alternated between +0 and +1 purely on sampling phase, and five of
# seven "CHIP REPORTED, NO PRESS" lines in the 11:05 window were this artefact.
# It was one hop too late, in the same family as bounding a sleep window one hop
# too early.
#
# So end the window BOUNDARY_MS before the press. A tap lost in the middle of a
# lift is still inside it; the press's own interrupt is not. The second gate
# below checks this directly: after the fix, an ordinary SHORT lift must read
# +0, and if short lifts still read +1 the boundary is still wrong.
BOUNDARY_MS = 40.0
VALIDATE_LIFTS = 30
...
out.write("== gap logger start %s  (v9: contacts counted by ABS_MT_TRACKING_ID, not BTN_TOUCH; boundary-corrected at %.0f ms)\n"
...
release_ts = None
frames_at_release = 0
pending_release_mark = False
hold_irq_at_press = None

contacts = 0             # ABS_MT_TRACKING_ID openings: the multi-touch truth
down = False
...
with open(DEV, "rb", buffering=0) as f:
    while True:
        ...
        if typ == EV_ABS and code == ABS_MT_TRACKING_ID:
            if val >= 0:
                contacts += 1
                out.write("%s  CONTACT #%d  (slot open)\n" % (wall(ts), contacts))
            continue
        ...
        if typ == EV_KEY and code == BTN_TOUCH:
            ...
            if down:
                ...
                if release_ts is not None:
                    ...
                    gating = lifts_seen < VALIDATE_LIFTS
                    lifts_seen += 1
                    if gating or (lift >= LIFT_MIN_MS and lifts
                                      and lift >= LIFT_FACTOR * max(med, 1.0)):
                        n, d, a, b = lift_evidence(release_ts,
                                                   ts - BOUNDARY_MS / 1000.0)
                        fr = frames - frames_at_release
                        # ☠️ Order matters, and v6 had it wrong. Frames first:
                        # if anything was delivered during the lift it was not a
                        # silent lift at all. Only then does the counter mean
                        # something, and only in ONE direction - a rise with no
                        # press is real evidence, while no rise is NOT a verdict
                        # about a lost tap. It cannot distinguish "the chip never
                        # sensed the finger" from "there was no finger": only the
                        # operator knows which, which is what the power-button
                        # marker is for. Saying "CHIP NEVER REPORTED" there, as
                        # v6 did, states as measured something nobody measured.
                        if d is None:
                            verdict = "no IRQ samples in the window"
                        elif fr > 0:
                            verdict = "frames delivered inside the lift"
                        elif d > 0:
                            verdict = "CHIP REPORTED, NO PRESS"
                        else:
                            verdict = "quiet lift - no evidence either way"
                ...
                # ── the gate: a hold is the known positive
                if presses <= VALIDATE_FIRST and press_ts is not None:
                    n, d, a, b = lift_evidence(press_ts, ts)
                    ok = (d is not None and d > 0)
                    out.write("%s  GATE hold #%d  irqs=%s  samples=%d  -> %s\n"
                              % (wall(ts), presses,
                                 "?" if d is None else "+%d" % d, n,
                                 "ok" if ok else
                                 "☠️ BROKEN: a delivered hold with no interrupts"))
            if down:
                press_ts = ts
            else:
                release_ts = ts
                # ☠️ NOT frames_at_release = frames here: the release's own
                # SYN_REPORT has not arrived yet, so it would be counted as a
                # frame "inside the lift" and every single lift would report
                # frames=1. That constant made v6's frames half meaningless.
                pending_release_mark = True
            continue

        if typ == EV_SYN and code == SYN_REPORT:
            frames += 1
            if pending_release_mark:
        ...
        now = time.time()
        if now - last_beat >= 60:
            out.write("%s  heartbeat: frames=%d presses=%d contacts=%d irqs=%s"
                      " gaps>=%.0fms=%d down=%s\n"
                      % (wall(now), frames, presses, contacts, irq_count(),
                         GAP_MS, gaps, down))
            last_beat = now
