...
- [x] **167.** Parse the MBN values, not just the item names - does the Hungarian config actually prefer PS voice?  — closed 2026-09-05 05:44
      lane: phone
      why: ☠️ THE LOAD-BEARING STEP OF THE MBN HYPOTHESIS, and it is unverified. Measured 2026-09-05: all three candidate configs (common/row/commerci, eu/vodafone/commerci/hungary, eu/vodafone/volte/global) contain the SAME NV item paths, including voice_domain_pref, qp_ims_service_enablement_config and IMSVoiceDynamicConfig - so the difference between them is in the item VALUES, which are binary. A comm diff appeared to show the items themselves differed and would have been a complete explanation
      it had warned 'input is not in sorted order' and was wrong. Needs an MBN parser (QPST, mbn_tools, or the open mcfg parsers) run over the three files, which are already copied off the device. The question to answer: does eu/vodafone/commerci/hungary set voice_domain_pref to a PS-preferring value and enable MMTEL voice in qp_ims_service_enablement_config, where ROW does not? A yes makes #166 worth running
      a no kills the hypothesis without touching the modem at all. ☠️ This is desk work on three files - no device, no risk - so do it BEFORE #166. Evidence: captures/2026-09-05_163-same-card-two-devices/README.md

- [x] **168.** The QTI IMS extension speaks HAL 1.2; the FP3's firmware offers up to 1.6 - does voice need the newer interface?  — closed 2026-09-05 08:39
      lane: phone
      why: ☠️ A device-side, source-level gap of exactly the kind #163 pointed at. Measured 2026-09-05: ofono connects to vendor.qti.hardware.radio.ims@1.2::IImsRadio, and ofono-binder-plugin-ext-qti cannot do better - qti_radio_ext.c implements only the 1.0/1.1/1.2 request sets and its version table tries 1.2, 1.1, 1.0. The device ships /vendor/lib64/vendor.qti.hardware.radio.ims@1.{3,4,5,6}.so. So the open-source extension is four interface versions behind the firmware. WHAT TO FIND OUT, in order: (1) which IMS service versions are actually REGISTERED on this device, not just which .so files exist - a library on disk is not a running service
      (2) what the 1.3-1.6 interfaces add over 1.2, from the vendor HIDL, specifically anything about MMTEL, media feature tags, or registration capabilities
      (3) only then whether extending the plugin is worth it. ☠️ Not offered as the cause: the ext registers IMS successfully at 1.2 and returns a valid P-Associated-URI, so 1.2 is clearly enough to register. The question is whether it is enough to be granted voice. The published UBports recipe (install ofono-binder-plugin-ext-qti, extPlugin = qti) is ALREADY applied on this device, and the forum example's radioInterface = 1.5 is the IRadio version, not the IMS one, so it is not a lever

- [x] **170.** Compare libssc against our SMGR sensor path - transport, API shape and iio-sensor-proxy integration  — closed 2026-09-05 09:19
      lane: phone
      why: The only actively maintained FOSS project in this space, found 2026-09-05 while checking imsd: codeberg.org/DylanVanAssche/libssc, C, ~1.1 MB, meson, src/ tests/ mocking/ docs/, GPLv3, updated 2026-07-27 - and unlike imsd it HAS CODE. ☠️ CORRECTION to this task's first version, which said the FP3 uses SMGR 'not SSC, different framework, different era'. That is wrong: docs/sensors/README.md states every sensor on the FP3 hangs off the SSC, a protection domain inside the ADSP, reached through the Sensor Manager QMI service over QRTR. So BOTH target the Snapdragon Sensor Core; what may differ is the service and protocol generation (our SMGR / QMI 256 + snsregd QMI 0x10F, versus whatever libssc speaks on 2018+ parts). Establishing exactly where they diverge IS the task, not a premise. Overlap to assess: (1) transport - QMI over QRTR, the same bus; (2) API shape - it exposes sensors as a GLib library and our path goes kernel IIO instead, so this is a genuine architectural fork worth understanding; (3) both end at iio-sensor-proxy. Read docs/sensors/README.md first. ☠️ A negative answer is a fine outcome and should be written down as one - the value is in not reinventing the QRTR/QMI plumbing and the test harness if they transfer
      (2) API shape - it exposes sensors as a GLib library, and our work has no settled shape yet
      (3) integration target - it plugs into iio-sensor-proxy, which is where our sensors would want to land. Read docs/sensors/README.md FIRST so the comparison is against what we actually do, then say concretely what is reusable and what is not. ☠️ A negative answer is a fine outcome and should be written down as one - the value is in not reinventing the QRTR/QMI plumbing and the test harness if they transfer, not in adopting the project

- [x] **171.** Does stopping the IMS PDN loop actually move the duty? Run the band-pinned A/B/A' ladder  — closed 2026-09-05 09:53
      lane: phone
      after: 135
...
- [x] **155.** #142 ROOT CAUSE: give the touchscreen the two supplies it is missing — add vcc_i2c-supply = <&pm8953_l6> and vdd-ana-supply = <&pm8953_l10> to touchscreen@48 in sdm632-fairphone-fp3.dts, teach himax_hx83112b to devm_regulator_get and enable them (and disable on remove), then re-run 142-trigger.sh  — closed 2026-09-05 11:40
      continues: 142
      lane: phone
...
- [x] **176.** Turn on the kernel's IPsec/ESP options so an IMS security association is even possible: CONFIG_XFRM_USER, CONFIG_INET_ESP, CONFIG_INET6_ESP, CONFIG_XFRM_ALGO (+ CONFIG_NET_KEY if wanted) in config-fp3.aarch64 - all currently 'is not set', and 'ip xfrm state' fails on the device. Verify after the flash with 'ip xfrm state' returning success. ☠️ Needs no radio, no test call and no operator: it either works after the flash or it does not. Everything about the imsd path is blocked behind it (docs/power/bringup/leads/imsd-is-code-now.md)  — closed 2026-09-05 13:46
      prio: 20 lane: phone

- [x] **175.** #142/#155 FIELD CONFIRMATION: use the phone normally for a session with pauses (>= ~36 min of real tapping - the measured gaps between stalls run to 726 s, so anything shorter can come back clean while the fault is fully present), then run fp3-selftest --only touch. The mechanism is already fixed and measured on r81 (l6 keeps a voter across display-off, l10 has a consumer); this is the only thing that can speak to the operator-visible 'Failed to read input event: -110'.  — closed 2026-09-05 16:27
      prio: 10
      lane: phone
...
- [x] **166.** Load the Hungarian carrier MBN into the modem and see whether VoLTE appears  — closed 2026-09-06 13:02
      lane: phone
      why: ☠️ WEAKENED 2026-09-05, NOT blocked - and an earlier note on this item today was wrong twice, see captures/2026-09-05_166-volte-config-activate/. The MBN theory says the generic ROW_Commercial withholds VoLTE; the two things a Hungarian config would fix already read correctly on this phone - the network grants this UE IMS voice (nas-get-system-info: 'IMS voice support: yes', MCC 216 MNC 70) and the modem is already ps-preferred. So the config was deliberately NOT changed: swapping it to chase a symptom the measurements attribute elsewhere would record a coincidence as a cause. It stays cheap and reversible if a reason appears (activate software,39:E0:BB:1B:94:EC:96:3E:4E:99:1D:DF:9C:7C:C4:4E:E1:B9:59:04 for Global-VoLTE-Vodafone; restore ROW_Commercial with 5C:F9:CA:DA:5C:35:85:17:BA:3B:B8:88:D0:34:2B:79:BD:5F:AD:ED). ☠️ The --pdc-load-config segfault is NOT the blocker either: the modem already holds 25 configs, so this is an activate, a different code path
      they-do: ☠️ AUTHORISED BY THE OPERATOR 2026-09-05 ('#178 után jöhet #166'). The go-ahead this task was waiting for is GIVEN, and the phone is already on pmOS, so both original blockers are gone; what remains is the ORDER (after #178) and one thing only a person can do: place the test call. Run it in this order and stop at any step that surprises you: (1) record the full before-state - pdc config list with statuses - so the current ROW_Commercial can be restored, and save it into the capture BEFORE loading anything; (2) load vodafone/commerci/hungary; (3) activate; (4) re-run the same call measurement that gave EDGE, with the operator placing the call. ☠️ It writes the modem's own persistent storage, which is shared by both slots and survives reboots and reflashes of either OS - so the before-state capture is not paperwork, it is the only way back
      after: 178
      prio: 20
      until: a qmicli that can SEND IMS 0x008f (Set IMS Services Enabled Setting) - the message is in libqmi's own data/qmi-service-ims.json since 1.38 with 0x10 = IMS Voice Over LTE Enable, but no qmicli exposes a CLI option for it (checked 1.39.0 on the phone and the 1.39.1 checkout). Every IMS service in the modem currently reads 'no' and IMS is 'not-registered', while the network grants IMS voice and the modem is already ps-preferred - so the switch is here, not in the carrier config
...
- [x] **179.** Verify on the device that a touch fault no longer cascades: a -110 must not be followed by a -6, an -EIO storm must not end in "Disabling IRQ", and neither must need a rebind --why ☠️ OPERATOR DECISION 2026-09-05: shortening the stall from 15.1 s to 2.0 s is NOT the fix - the fault itself must go. Two commits added on top of #157: i2c-qup now decodes QUP_I2C_STATUS on timeout (SDA/SCL/master bits, from the vendor i2c-msm-v2 register map, confirmed against our own 0x00138700 hung / 0x0c000000 idle capture) and issues the hardware bus-clear (QUP_I2C_MASTER_BUS_CLR 0x40c, nine pulses) when the status says the bus is held. Mainline i2c-qup has NO recovery of any kind; the vendor driver on this same silicon has had one all along. Success criterion is the SIGNATURE, not the count: on r82 every -110 was followed within the same second by a -6, and one wedge needed a driver rebind. ☠️ The unbind reproducer 142-trigger.sh CANNOT judge this and must not be used for it - it drops the driver's regulator votes and so is blind to the supply fix too --lane phone  — closed 2026-09-06 20:55
      they-do: USE THE PHONE - at least 36 minutes of actual touching, spread out rather than in one burst, because the r82 stalls were 23-726 s apart. Nothing needs setting up. ☠️ This note deliberately names NO release and NO log filename: it was rewritten twice in one day because flashing invalidated it. Ask the device instead: 'fp3-ssh "uname -v; cat /usr/share/kernel/fp3/fp3-commit"' for what is running, 'ls -t /var/log/fp3-touch/*.tsv | head -1' for the current sampler log, and analyse it with touch-exposure.py from captures/2026-09-05_178-retry-rate-on-r83/, which reports ACTIVE minutes as the denominator and refuses a verdict below 36. ☠️ Judge the SIGNATURE, not the count: on r82 every -110 was followed within the same second by a -6 and one wedge needed a rebind, so a -110 with no -6 after it is the SUCCESS case. And after 2026-09-06 also watch whether an -EIO storm still ends in 'Disabling IRQ' - r85 is supposed to make it not
      after: 180

- [x] **182.** ModemManager loses the modem across suspend/resume: on wake it rebuilds its device list, finds no QMI port and gives up without retrying - 'couldn't create modem for device qcom-soc: Unsupported device: at least a QMI port is required'. The phone then has NO cellular service until ModemManager is restarted, while the modem firmware is fine (three remoteprocs running, qmicli --dms-get-model answers). Measured 2026-09-06 23:23: suspended 23:04:02, woken by an incoming call 23:23:38, and the call could not ring because there was no modem. Fix candidates, cheapest first: (1) find out WHY the qrtr QMI port is not there at probe time on resume - is the qrtr node re-created late, or does MM probe before qrtr comes back; (2) if it is a race, a systemd unit After=suspend.target that restarts ModemManager is a workaround, NOT a fix, and it costs the first-call latency #181 wants to measure; (3) upstream: MM should retry the probe rather than concluding unsupported. ☠️ #181 CANNOT run until this is fixed - its morning sample would be a guaranteed 'did not ring' read as a reachability result --lane phone --prio 15  — closed 2026-09-07 06:31
      when: whenever the operator can place ONE call at the end of a long suspend - it is the only arm that fails
      they-do: ONE incoming call at the end of a ~12 minute suspend, to prove the fix. ★ CAUSE FOUND 2026-09-07 and it is ours: pmOS ships /usr/lib/systemd/system/ModemManager.service.d/quick-suspend-resume.conf forcing --test-quick-suspend-resume, which makes MM SYNC the existing modem on resume instead of re-scanning from scratch - and our own /etc/.../zz-fp3-debug.conf cleared ExecStart and set only --log-level=DEBUG, so /etc beat /usr/lib and the flag was gone. The drop-in now carries BOTH flags. What is left is the proof: arm the suspend, one call, does it ring. ☠️ Until that run, this is a hypothesis with a mechanism, not a result. Analysis: docs/power/bringup/captures/2026-09-06_182-modem-lost-on-resume/
      why: DIAGNOSED 2026-09-07 00:16, the fix is not written. Reproduced with a 10 min 23 s suspend ended by an incoming call: MM probes at +4 s, finds no QMI port, concludes 'Unsupported device' and never re-probes - while qmicli -d qrtr://0 --dms-get-model answered normally ~30 s after resume with nothing restarted. So the port is LATE, not missing: a race. 24 s survives; 10 min 23 s and 19 min 36 s do not. Next, cheapest first: (a) bracket HOW late - one suspend with a poll loop from +0 s, which is the number a retry needs; (b) decide between the real fix (make MM retry instead of concluding unsupported - upstream shaped) and the workaround (a unit After=suspend.target restarting MM, which costs exactly the first-call latency #181 measures, so it must not be shipped silently). Capture: docs/power/bringup/captures/2026-09-06_182-modem-lost-on-resume/
      until: fp3-qmi-late.service finishes on the device - ARMED 2026-09-07 00:25:11, three 12-minute rtcwake suspends, expected done about 01:05. It measures step (a): how many seconds after resume the QMI transport first answers, polled from +0 s, and whether a 12-minute suspend loses the modem each time. Log: /var/log/fp3-qmi-late.log. ☠️ It restarts ModemManager after any cycle that loses the modem and once at the end, so the phone is callable in the morning whatever happens. ☠️ Earlier rtcwake runs woke EARLY (600 s asked, ~100 s slept) for a reason not understood - one of those was the operator's call, the other was not explained - so check the wall-clock 'resumed after N s' lines before trusting a cycle
