...
## ⟲ Resume — autonomous run, generated 2026-09-01 22:19 UTC
...
GOAL: pmOS fogyasztás felezése (≤50 mA) UT reagálási paraméterei mellett — a 8 órás modem-census (86 kör) eredményének feldolgozása és a következő lépések
  ...
  […] 47. 46 PRONTO passzazsr: minden jovobeli negy-master ablakban rogzitsd a TRANSZPORTOT (ssh-ut, wlan0 oper/carrier, rfkill) es nezd az elore lejegyzett josolatot: ha MPSS olcsoba megy, a PRONTO ~17-19%-ra es ~20 ms-ra ter vissza. Onallo ablakot NE kolts ra
        · egy jovobeli OLCSO (MPSS ~5%) negy-master ablakra - passzazsrkent, nem sajat meresre
  ...
  [~] 56. ★★★★★ AZ INTERVENCIO (elore rogzitve a capture-ben): allitsd meg az IMS-PDN hurkot es merd a dutyt. 1. kar: az AP FOGJA a PDN-t (mmcli --simple-connect apn=ims) - FUT; 2. kar ha ez nem eleg: az ims-profilok letiltasa/torlese. ELORE ROGZITVE: a duty ~5-6% fele esik ES az ebredes-utem 3,14/s-re ugrik => MEGNEVEZVE ES MEGERSITVE; valtozatlan duty => a hurok utas, a capture oksagi allitasa hamis
        · ★★★★★ 3. KAR: AZ IMS-KIKAPCSOLAS MEGALLITOTTA A HURKOT. Qmi.ClientIms.set_ims_services_enabled_setting (bindolva, sajat ims-toggle.py), minden kapcsolo False-ra, visszaolvasva. 120 s log utana: NULLA ESM-uzenet (elotte 220), RRC OTA 391 -> 179, osszmeret 38299 -> 9620 bajt. A telefon vegig 'registered'. ☠️ Csapda menet kozben: a set_ims_service_enabled(False) ONMAGABAN csak az UT-t billentette at - a setter- es getter-nevek NEM felelnek meg egymasnak, ezert minden kapcsolot kulon kell beallitani es VISSZAOLVASNI. Fut a 600 s duty-ablak: elore rogzitve ~5-6% es 3,14/s ebredes-utem
```
...
```
OUTSIDE REVIEW: last 2.2 h ago, 6 result(s) recorded since.
  · Review #8 VALASZ. (a) 'HAGYD ABBA az AP->modem jelzesek felsorolasat, az a ter LEZARULT' - es nem a census a legerosebb erv, hanem a TX-IN-TRAFFIC: a modem ADASBA megy nulla felhasznaloi adat mellett, es SEMMILYEN hianyzo AP-oldali bit vagy handshake nem valt ki OTA-adast (egy kiszolgalatlan lookup vagy be nem allitott flag lokalisan es olcson retryzik). ⇒ halozat-fele iranyulo eljarasok + a halozat exoneralva (orakulum, ugyanaz a SIM) = MODEM-BELSO kliensek modem-belso konfiguracion. DE a dichotomiam masik felet ELUTASITJA: 'az AP nem eri el' HAMIS - futasideju jelzes-kar nincs tobb, de bindolt QMI olvasas/iras (IMSA, IMSS, WDS attach-PDN, PDC), EFS/NV diagon at, es a ketoldali orakulum-diff MIND elerheto. A pivot: hianyzo AP-jel keresese HELYETT a modem sajat tortenetenek rogzitese. (b) az ADSP-atiranyitas VIGASZDIJ: flasheld OPPORTUNISZTIKUSAN, amikor ugyis flashelunk, NULLA dedikalt ablakot ne kolts ra; a duty-front jelenleg SEMMILYEN kernel-valtozast nem igenyel, minden nyitott kerdest OLVASASOK valaszolnak meg. (c) a bindolt IMS-olvasas MEGERI, NEGY elore rogzitett aggal (nem-regisztralt/retry => a beavatkozas a bizonyitek, nem a konzisztencia; disabled/not-provisioned => az IMS-retry HALOTT; registered => IMS keepalive, es kerdezd meg ugyanezt az orakulumon; tovabbra is elutasit => 'QMI-vel olvashatatlan', es NE csinalj belole bizonyitekot). (d) TULOLVASAS-AUDIT: az 'ADSP pontosan egy bitre iratkozott fel' KEVESBE jelentos mint amilyennek tunik - egy maszk azt tukrozi, mit hasznal a firmware-image, nem azt hogy a bit fontos. ES egy nuansz a 30 masodperces gyilkolasra: a maszk az INTERRUPTOKAT szabalyozza, egy tavoli fel POLLOZHATJA a masik SMSM-allapotat - tehat a helyes fogalmazas 'nem tudja EBRESZTENI a modemet, es olvasasi uton sem marad mechanizmus a nulla-IPC census utan', NEM 'a modem nem latja'. ☠️☠️ EGY TARGYI TEVEDESE: azt irja, a PDC-t 'soha nem olvastam' - DE IGEN, a 25. tetelben (2026-09-01 17:30, aktiv ROW_Commercial, 25 software-config), es a WDS profil-listat is ma este. Ami TENYLEG hianyzik: az attach-PDN lista. ☠️ A kert 'egy asztali perc' ellenorzes LEFUTOTT es a lyuk ZARVA: a census awk-ja src_port szerint bucketel SZURES NELKUL (ismeretlen port 'portN'-kent jelenne meg), es a ver!=1 eldobas szamlaloja NEM sult el ⇒ ha jott volna qrtr control-forgalom (NEW_LOOKUP/NEW_SERVER), latszott volna. Nem jott
```

**☠️ Results have landed that the plan does not mention yet:**

  - uncommitted changes under /mnt/1TB/pmos/fp3-pmaports/docs:
     ?? docs/power/bringup/captures/2026-09-02_diag-ota-pmos/raw/diag-ims-held.bin
     ?? docs/power/bringup/captures/2026-09-02_diag-ota-pmos/raw/diag-ims-off.bin
     ?? docs/power/bringup/tools/ims-toggle.py
