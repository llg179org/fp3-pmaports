...
## ⟲ Resume — autonomous run, generated 2026-09-01 19:41 UTC
...
GOAL: pmOS fogyasztás felezése (≤50 mA) UT reagálási paraméterei mellett — a 8 órás modem-census (86 kör) eredményének feldolgozása és a következő lépések
  ...
  [~] 50. ★★★★★ SMSM PROC_AWAKE (12. bit) - FORRASBOL TALALT ASZIMMETRIA: az orakulum bootkor 1-re allitja es csak suspend alatt nullazza (msm_smd.c 2450: sajat notifier PM_POST_SUSPEND-del INIT elott), a mainline smsm.c SOHA nem nyul hozza es egyetlen DT sem fogyasztja a <&apps_smsm 12>-t ⇒ pmOS-en VEGIG nulla. Kategoria: power. Implementacio: consumer kell, nem uj vezetek (smsm_update_bits mar qcom_smem_state provider, a wcnss_wifi a 9/10 bitet hajtja rajta). Kiserlet: 600 s A/B, sav pinelve, kovariansokkal. ☠️ ELORE ROGZITVE: a duty erdemben esik ~6% fele => megvan a mechanizmus es szallithato fix; valtozatlan a ~3 pontos ismetelhetosegen belul => ez a firmware nem olvassa a bitet, a lead HALOTT
        · PATCH MEGIRVA es commitolva mind a harom retegen (wip/7.1.3/power 407669069a90+bca898cde190, integration a62b48cdc3ec, debug-int b8023520cddb) - ket commit, driver+DT szetvalasztva a submit-konvencio szerint. A bitszam DT-property (qcom,proc-awake-bit), nem beegetve, tehat minden mas platform viselkedese valtozatlan. checkpatch: 0 hiba (a 4 warning a Co-authored-by trailer, ami a lokal fork-konvencio). ☠️ Ket sajat hibat talaltam a sajat diffemben olvasas kozben: (1) a blokk eloszor a probe KOZEPEN volt, ahol a kesobbi hibaag ott hagyta volna a regisztralt PM-notifiert egy felszabaditott objektumon; (2) miutan a vegere mozgattam, a 'goto out_put' ATUGROTTA volna az unwind_interfaces-t. Mindketto javitva. NINCS pusholva es NINCS buildelve-flashelve - a meres az ejszakai ablak utan
  [ ] 51. ★★ RPM SLEEP-SET (a masodik forras-lead): a vendor kulon szavaz az active es a sleep setre (rpm-smd.c + az egesz msm_bus stack); a mainline smd-rpm.c ISMERI a fogalmat (a request active/sleep state flageket visz), de hogy a mi drivereink toltik-e valaha a sleep setet, az kulon kerdes. Alacsonyabb prior egy PER-EBREDESES 125 ms-os koltsegre, ezert a masodik
```
...
```
OUTSIDE REVIEW: last 1.2 h ago, 4 result(s) recorded since.
  · Fable review #7 (2026-09-01 este): (a) PRONTO = labjegyzet, ne kolts ra ablakot; uj confound: a meres TRANSZPORTJA maga — egy wifi-s ssh session AZ a PRONTO ebrenlet. (b) semmi nem szol a modem-belso VEGREHAJTAS ellen, de a 'modem-internal' nem csuszhat at 'nem a mi bugunk'-ba: a vezeto okok olyanok, amiket a mi stackunk NEM csinal meg. (c) ejszakai bomlas-figyelo teljes terve (10 s nyers 4-master, 10 perc kovariansok, journal-kurzor, rmnet elott/utan, MM fut, nulla radio-iras, >=10 h) elore rogzitett olvasatokkal. (d) strukturalis javitas kotelezo: EGY kanonikus RPM-stats olvaso + golden-file onteszt + modem-window.sh stdout. (e) ne idezd tamogatasként hogy 'ezt josoltam' — egy n=1 confoundolt ablak nem megerosites. Sorrend ma estere: imsa olvasas -> ejszakai ablak -> 08-31 journal-kontextus.
```
