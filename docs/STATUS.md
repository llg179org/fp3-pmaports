...
## ⟲ Resume — autonomous run, generated 2026-09-01 13:29 UTC
...
GOAL: pmOS fogyasztás felezése (≤50 mA) UT reagálási paraméterei mellett — a 8 órás modem-census (86 kör) eredményének feldolgozása és a következő lépések
  ...
  [~] 15. ★★★★★ PERZISZTENS ALLAPOT: ez az egyetlen kategoria, ami tulel daemont, WiFit, kabelt, modem-power-cyclet, rebootot ES napszakot. Ket alcsoport, es szetvalaszthatok: (a) MODEM NV / carrier config - amit 08-31 reggel valami beleirt; (b) a HALOZAT allapota erre az IMSI-re. Szetvalasztas: detach+attach (a halozati oldalt ujraalkuja), illetve a modem NV/carrier-config olvasasa qmiclivel es osszevetes azzal, amit a leads/modem-carrier-config.md mar tud
        · REVIEW ATFOGALMAZTA: a detach+attach LEKERUL (a 12. tetel modem-mag ciklusa mar ujra-attacholt es semmi nem mozdult ⇒ per-attach halozati alku kizarva). A generikus 'NV-olvasas' helyett konkret muszerek: PDC aktiv carrier-config, system-selection-preference (RAT-lista/acquisition order), IMS-reg allapot, EF_FPLMN a SIM-rol. ☠️ ES a kategoria maga is gyanus: lehet, hogy nem keszulek-oldali perzisztens allapot, hanem HALOZATI regime - ezt a 23. tetel donti el
