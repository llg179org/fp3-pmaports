...
## ⟲ Resume — autonomous run, generated 2026-09-02 09:14 UTC
...
GOAL: pmOS fogyasztás felezése (≤50 mA) UT reagálási paraméterei mellett — a 8 órás modem-census (86 kör) eredményének feldolgozása és a következő lépések
  ...
  [ ] 33. ★★ A PONTOSITOTT ADSP-KERDES: mi tortenik az ADSP-vel a SUSPEND alatt, ha a ModemManager fut? A kulonbseg csak ott letezik. Jelolt: a daemon suspend-handshake-je (terse) valamit felebreszt vagy nyitva hagy a freeze pillanataban. Muszer: LPASS-szamlalo a suspend ket oldalan, MM futo/leallitott karral, UGYANABBAN a bootban - ez mar a modem-night.sh formatuma, csak 2x2 kor kell hozza
        · ☠️ THE WAIT EXPIRED (40m) — re-check the reason before acting: a 64. capture-je hasznalja a telefont 11:07-ig; ket meres egy telefonon mindkettot elrontja. Utana azonnal indithato (2x2 x 600 s suspend-kor).
  ...
  [?] 63. ★★★★★ ELERHETOSEG-TESZT VEGLEGES FORMAJA (Fable #13 b) - a 60. tetel elesitese: (1) COLD BOOT a boot-asszertalo szolgaltatassal, NE kezzel epitett allapotbol (a MM-nem-enumeral lelet mutatta, hogy a kezi es a bootolt allapot nem ugyanaz a rendszer); (2) ★ KORONA-TESZT: alvo AP + akkun + IMS off + bejovo hivas -> felebred ES cseng; (3) reboot-ateles: az iras perzisztens, tehat reboot utan is csengenie kell; (4) SMS mindket irany kulon tetel (IMS-rol SGs-re esik vissza - atkapcsolas, nem magatol ertetodes). ☠️ A CS-attach/SGs csak PREDIKTOR; kapukent egyedul a csenges fogadhato el. ☠️ Diag-paging-megfigyelessel NE valtsd ki - a maszk kontaminal
        · Az orankenti koz ERI EL a veszelyes sarkot (paging-miss hosszu tetlenseg es mely alvas utan), amit a mai negy hivas percnyi kozokkel nem erintett.
  ...
  […] 81. ★★★★ A MERES-INDITO WRAPPER (Fable #4 b): a PreToolUse-kapu mintazat-alapu es CSAK a sajat sessionomben fut - egy masik terminal kezi inditasat nem latja. A helyes forma EGYETLEN belepesi pont (fp3-measure wrapper), ami maga olvassa a consulting-allapotot ES egy ESZKOZ-OLDALI zarat ⇒ az ervenyesites GEP-SZINTU lesz, nem session-szintu, es egyszerre zarja a tobb-gep- es a kezi-ssh-lyukat. A wrapper kovetelhet ETA-t is (a meres+figyelo=egy-objektum szabaly odakoltozik). A PreToolUse szerepe utana SZUK: a nyers indito-formak (systemd-run/nohup/setsid) tiltasa 'a wrapperen at' uzenettel - nem a hosszu munkat ismeri fel, hanem a wrapper MEGKERULESET
        · az eszkoz-oldali lockfile-hoz szabad telefon kell; most a 64. capture fut. A ket session-oldali resz (wrapper + bypass-kapu) KESZ es megmutatva, es a wrapper azota ket ujabb kaput kapott (RuntimeMaxSec<ETA elutasitas, unit-nevadas).
  ...
  […] 85. ★★★★★ REPLIKACIO 3 BOOTON, 2 NAPON + OCV-VS-QG (Fable 5.1 c, ejszakai terv): 3x { reboot -> reconciler konvergal (naplobol bizonyitva) -> radio-ki rest + OCV -> B-cenzus 90 s-os ebresztovel 60-90 perc -> rest + OCV }, hajnalban egy A-lab kontroll. Egyszerre adja: a 40 mA replikaciojat 3 booton (a visszatero hibamintank ellenszere: egy boot allapota != a telefon tulajdonsaga), az OCV-vs-QG offset-korlatot, a reconciler boot-utani viselkedesenek 3x eles bizonyitekat, es 90 s-nal labankent 2-3x tobb megtartott ablakot.
        · estere: a KULSO OCV-PAR fogja kozre az egesz ejszakat (este rest+OCV, hajnalban rest+OCV), a 3-bootos replikacio a QG-labakbol jon kozben
...
OUTSIDE REVIEW: last 0.0 h ago, 0 result(s) recorded since.
  · Fable #7. Szamok ellenorizve: A-ra s=12,2*sqrt(7)/2,447=13,2 mA, A'-re s=43,3*sqrt(11)/2,228=64,5 mA - stimmel. ☠️ FABLE SAJAT MAGAT CAFOLJA: a #6-os 'n<15-nel t-sav' tanacsa EGY ROSSZABB BECSLORE cserelt, mert a bootstrap a cnt-SULYOZOTT aggregatumot mintavetelezte ujra (a kis-cnt-s zajos ablakok kis sulyt kapnak), a t-sav viszont a SULYOZATLAN ablak-atlagokon fut, ahol ugyanezek dominaljak a szorast. A helyes: a sulyozott atlag SULYOZOTT VARIANCIAJA, Var(mu)=sum(w_i^2*(x_i-mu)^2)/(sum w_i)^2, w=cnt. ES az A' s=64,5 tizenegy ablakbol azt jelenti, hogy 1-2 ablak valoszinuleg 200 mA folott van - SZEMMEL meg kell nezni, mielott statisztikanak konyvelem. (a) A NEMA DIAG: ☠️ a maszk-iras ACK-ja FUGGETLEN a maszk tartalmatol (a DIAG parancs-valaszokat is visz, nem csak log-csomagot), tehat a kontroll +0-ja azt jelenti, hogy MEG AZ ACK SEM jott => a baj a SZALLITASI/SESSION retegben van, NEM a maszkban - a hipotezisem mar a sajat kontrollom alapjan valoszinutlen volt. Letra: (1) kotelezoen valaszolo keres (Version 0x00 vagy build-ID 0x7C): valasz jon => csatorna+olvaso el mindket iranyban, akkor olvasd VISSZA a log-maszkot (0x73 GET) - ha nullan van a sajat irasom utan, valami reseteli; ha be van allitva de nincs log, session-gyanu. Valasz NEM jon => a TX-ut halott (es ehhez PASSZOL, hogy a nyitasi burst atjott: RX el, TX nem) - dmesg glink/rpmsg hiba, flow-control kredit. (2) fuser/lsof a diag-eszkozon: maradt-e nyitva korabbi capture-kliens. (3) close/reopen. (4) DIAG mode-reset (0x29) - a diag-taskot inditja ujra, NEM a modemet. (5) remoteproc-restart csak legvegul, a hivas-census szunetében. (b) A NAPPALI ORANKENTI CENSUS a ratat gyujti, DE A VESZELYES SARKOT NEM MERI: minden hivas ujrainditja a tetlenseg orajat => az '1 oras idle'-t surun mintavetelezi, a '8 orasat' SOHA. Ingyen begyujtheto: NAPI EGY HIVAS REGGEL, az elso interakcio ELOTT - az az ejszakai 7-9 oras sarok, nulla tulajdonosi teherrel. Napi 10 nappali hivassal 6 nap alatt N=60 => p>=0,95. KET CSAPDA: (i) a sajat kiserleteim konfundalnak - kell kereszt-annotacio, hogy a karbantartasi ablakban erkezo slot ne szamitson hibanak; (ii) a KIMARADT hivas definicio szerint NEM hagy nyomot a ringlogban, tehat a kezbesitest a MENETREND es a naplo kulonbsege adja - ehhez fix hh:mm slotok kellenek ES megbizhato idobelyeg, DE A FEJLESZTOI TELEFON RTC-JE 1970-rol indul (sajat memoriam!), tehat a ringlog naplozzon MONOTON IDOT + BOOT-ID-t is. (c) HAROM TETEL, AMIT KESZNEK HISZEK, DE NEM AZ: (1) az '51-59 mA' focim ELAVULT a sajat uj savjaim mogott - a becsuletes alak A-B = 91,0-40,1 = 50,9, es a sav az A-lab +-12,2-jet orokli; a jelentes MINDEN szama a ma3-fit --md kimenetebol regeneralodjon, prozaban masolt szam ne legyen; (2) a boot-utani VISSZAIRO kilete NYITOTT - a reconciler mukodo TUNETKEZELES, a jelentes ne allitsa, hogy a mechanizmus ertett: 'mitigalva, nem attribualva'; (3) MINDEN mA- es duty-szam scope-ja eutran-1, EGYETLEN cella - a sav 54 mA-t er, tehat ha a telefon normal uzemben nem eutran-1-en tabooroz, a TELEPITETT nyereseg mas szam. (d) A LEGGYENGEBB LANCSZEM: a ma esti ejszakai futas EGYLOVETU, felugyeletlen menet, aminek szinte minden alkatresze MA keszult es SOSEM futott vegig egyben (ma3-fit ma valtozott, a wrapper ETA-kapuja ma epult, a foglaltsag-ellenorzesrol MA derult ki, hogy mindent blokkolt volna - csak eles proban). Kell egy 20 PERCES MINIATUR FOPROBA a TELJES lancbol lefekves elott: 1 reboot -> reconciler-konvergencia -> radio-ki rest + OCV -> 10 perces B-lab 90 s-os ebresztovel -> ma3-fit --md, a wrapper es a session-zar alatt. Ha atmegy, az ejszaka meres; ha nem, egy estet nyertem egy ejszaka helyett.
```
