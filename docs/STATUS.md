...
## ⟲ Resume — autonomous run, generated 2026-08-31 06:07 UTC
...
```
GOAL: pmOS fogyasztás felezése (≤50 mA) UT reagálási paraméterei mellett — a keret: fp3-pmaports/docs/power/NEXT-RUN.md
  […] 1. ★ STEP 0 (mindent blokkol): MENNYIT FOGYASZT A TELEFON ALVÁS KÖZBEN? sleep-night.sh + fit, ébren-kontroll ablakkal. ☠️ KÁBEL NÉLKÜL (töltés alatt a cur_mA a töltőáram). Előre rögzített olvasat: <15 mA ⇒ a residency a kar (R1); >40 mA ⇒ az AP nem megy le, X1 LESZ a cél; közte ⇒ X1 először
        · a meres ALL (floor_mA = 48 +/- 5); csak a MAGYARAZATA nyitott, es azt a 17. es 18. tetel viszi tovabb. Itt nincs ujrafuttatando
  [x] 2. ★ STEP X1 (a valódi front, csak ez megy az 54,9 mA intercept alá): miért nem éri el az RPM a vlow/vmin-t? rail-census.sh + parse, sleepset-witness.sh, vlow-probe.sh. A leszállítandó egy NÉVSOR: mely rail-ek szavaznak aktívra és sosem alvásra, és melyik driver szavazza. ☠️ üres census = műszer-eredmény, known-positive ellen validálni
        · ★★★★★ LEZARVA az ORAKULUM ellen: UT APSS xo_count=0x0, xo_accumulated_duration=0x0, xo_last_entered_at=0x0 — 120 s uptime-nal, numshutdowns=0xc28 (3112 power-collapse). PONTOSAN ugyanaz a minta, mint pmOS-en. ⇒ az 'APSS sose engedi el a kristalyt' NEM pmOS-hiba, hanem a platform normalis allapota egy 6,1%-os rendszeren is. Az X-palya (APSS/RPM melyebb szavazat) LEZARVA: amit uldoztunk, azt a legyozendo rendszer sem csinalja. ☠️ A vlow maga NEM eldontheto differencialisan: az orakulum kernelen rpm_stats NEM LETEZIK; a legmelyebb allapotara a lpm_stats system-pc=674 success / 6,63 s / ~300 s a tanu
  [x] 3. ★ STEP D1: AZ ORÁKULUM SAJÁT A/B-je, amit még SOHA nem futtattunk — slot_a-n modem-window.sh ofono kontextussal AKTÍV / DEAKTIVÁLT / aktív. radio-context.sh minden lábon. Előre rögzítve: ha ~35%-ra ugrik ⇒ a data-kontextus a különbség (és a pmOS-os lapos bearer-A/B nem ekvivalens kontextust mért); ha ~6% marad ⇒ attach-idejű konfiguráció (DRX/paging), és ez RETIRÁLJA a netmgrd/ipacm sztorit
        · ★★★★★ A KENYELMETLEN AG LETT IGAZ: MPSS awake 6,9% (2 kontextus) / 5,2% (1) / 5,4% (visszaallitva) - a duty NEM emelkedett, kicsit CSOKKENT, es az A' kozelebb van B-hez mint A-hoz ⇒ a szoras alapvonal-drift, nem a kar. Az orakulum 5-7% bearerrel ES anelkul, a pmOS 34,8% ⇒ A DATA-KONTEXTUS NEM A KULONBSEG, a netmgrd/ipacm sztori RETIRALVA (es a pmOS-os lapos bearer-A/B ezzel megerosodik, nem gyengul). ⇒ D1 = 'attach-ideju konfiguracio' ag ⇒ D2 a folytatas. ☠️ Ket gyengito megjegyzes rogzitve: a DeactivateAll az IMS-kontextust fennhagyta (1 kontextus 2 helyett, nem nulla), es a jel 30->20->22 driftelt (ez a megfigyelt IRANY ELLEN hat)
  […] 4. STEP D2 (csak ha D1 = attach-konfiguráció): MEGNEVEZNI mit csinál a modem 34,8%-on — DIAG úton (diag-handshake.py/diag-probe.py) RRC-állapot, paging cycle, DRX. ☠️ előbb az orákulumon igazolni, hogy a végpont a HELYES
        · az MM-stopped duty-ablak eredmenyere var (600 s, unit=mmstopduty): ha ~35%, minden userspace-magyarazat kizarva es a D2 a kovetkezo; ha ~6%, a mai fo eredmenyt kell ujragondolni. Utana sajat boot, bootonkent egy DIAG-probalkozas.
  [x] 5. R1a: a suspend-marker áthelyezése /usr/lib/systemd/system-sleep/-be (minden inhibitor UTÁN fut) + a QMI-census újrafuttatása. Amíg nincs meg: NYITOTT, hogy a modem küld-e bármit magától alvás közben — ma három azonos osztályú határhiba volt
        · ★★★★★ HARMADSZORRA JOL: a system-sleep hookkal hatarolt ablakban a 600/600 s-os, RIASZTASSAL vegzodo kor NULLA QMI-uzenetet tartalmaz, a 46 s-os, MODEM-ELLEL vegzodo kor pontosan EGYET. Ez az elore rogzitett joslat ⇒ minden s2idle alatt erkezo QMI-csomag MEGSZAKITJA az alvast; 'zaj, ami nem ebreszt' nincs. ☠️ n=2, es az az egy csomag nincs azonositva (a 68-as port nem volt a futas sajat portterkepeben ⇒ msg=3 negy szolgaltatasra illik). ☠️☠️ UJ CSAPDA: az ebreszto IRQ-szamok BOOTONKENT VANDOROLNAK - ezen a booton 56=pm8xxx_rtc_alarm, 141=smd-edge, es 72=camss_msm_csid1, mikozben a ma korabbi capture-ok a 72-t 'RTC'-nek olvassak
  [x] 6. R1b: idle-suspend-window.sh futtatása — kér-e a telefon MAGÁTÓL suspendet? ☠️ indítás után EL KELL ENGEDNI (az ssh maga sleep-inhibitor), a tanú a host USB-log
        · LEFUTOTT 2026-08-30 17:53-18:23: success 13->13 = NULLA suspend 1800 s alatt, IdleAction=suspend + 60 s kuszob mellett. ☠️ De MINDKET elore rogzitett olvasatom megdolt (ld. 13. tetel): a szamito session tetlent jelentett ES nem volt block-modu inhibitor. A harmadik ag - hogy a policy be sem toltodott - nyitva, es a 13-as meri.
  [-] 7. R1c: szelektív SMD wake-filter — CSAK terv, amíg R1a meg nem mondja, van-e mit szűrni. Ébresztő-lista MÉRVE: Voice(39)+WMS(51); zaj: NAS(40)/DSD(52)
        · PARKOLVA a lead sajat elore rogzitett szabalya szerint - ujranyitando csak a rovid-alvas regime sajat censusa utan
  […] 8. Higiénia: aw8898 SYSST-volatile javítás — VÁLASZ a listán az upstream aw8898-posztolásra (NEM a saját sorozatunkba)
        · kifele meno akcio az LKML-re = emberi dontes
  [x] 9. ★ SLOT-TRIP slot_a-ra (UT): EGY reboot, KET valasz — (X1) ut-vlow-idle.sh: eléri-e az orakulum a vlow/vmin-t? Ha 0 ⇒ AZ EGESZ X-palya ZARVA. (D1) modem-window.sh ofono-kontextus AKTIV/DEAKTIVALT/aktiv + radio-context.sh minden labon
        · Slot-trip LEZARVA: kimenet UT-ra 13:29, vissza pmOS-ra 14:00:32 (uptime 21 s, akku 98%). Ket valasz egy rebootbol: X1 negativ (X-palya zarva), D1 = attach-ideju konfiguracio ag (netmgrd/ipacm retiralva). ☠️ A qbootctl NINCS az UT-n (Alpine-eszkoz), tehat a visszaut slot-ellenorzese egytanus volt (gptattr sajat lemez-visszaolvasasa); a layoutot a reggeli pmOS-oldali qbootctl hitelesitette
  [x] 10. R1b UTAN (nem elotte, hogy ne zavarja az ablakot): KI IRTA a sleep-inactive-battery-type='nothing'-ot? Ket lehetoseg, egy olvasas kulonbozteti meg: 'dconf read /org/gnome/settings-daemon/plugins/power/sleep-inactive-battery-type' (ertek = user-szintu dconf-feluliras, egy sor visszaallitani) vs 'grep -rn sleep-inactive /usr/share/glib-2.0/schemas/' (ures dconf = csomag-szintu gschema override, az nevezi meg a csomagot). ☠️ Az upstream alapertek MERVE: mindket type 'suspend', mindket timeout 900 - tehat a timeout erintetlen, a type explicit atirva.
        · ★ MEGVAN: dconf(fp3) = 'nothing' => USER-SZINTU dconf-iras, egy sor visszaallitani (a dump csak ket kulcsot tartalmaz: battery-type='nothing', battery-timeout=900). Az AC-ag ezzel szemben CSOMAGBOL jon: 00_postmarketos-base-ui-gnome.gschema.override 2. sor. ☠️ A 'nothing' NEM tiltja az alvast, csak nem kezdemenyezi - tehat NEM ez okozta az R1b nullat. ☠️ Muszer-csapda: a gsettings/dconf a FUTTATO felhasznalo adatbazisat olvassa; az R1b rootkent futott es ezert a sema-alapertelmezest ('suspend') jelentette ugyanarra a kulcsra egy perccel kesobb.
  […] 11. FELTETELES (csak ha a STEP 0 a 15-40 mA savba esik): MENNYIT VISZ MAGA AZ USB-LINK? A meres nem huzza ki a kabelt, csak a PMIC input_suspend bitet allitja - a PHY bekapcsolva marad es a CDC-NCM enumeralva, mert a kabel A LINK. A floor_mA tehat tartalmazza az USB koltseget, ami mA-ben SOHA nem volt megmerve (a 2026-08-24-i capture csak annyit mond, hogy a vezerlo es a PHY runtime-suspendbe megy). A <15 es a >40 agat nem billenti at, a kozepsot igen. Eszkoz: usb-off-census.sh / lpass-usb-ab.sh mintajara egy A/B a PHY unbindolasaval - de az elvagja a linket, tehat ora-alapu, oninditos meresnek kell lennie.
        · ugyanaz: sorbaallitva a duty-ablak moge
  [-] 12. FELTETELES (csak ha az R1b delta=0 ES IdleHint=no): R1b-2 — ugyanaz az ablak, de a kijelzot elobb elsotetitve (bl_power=4), hogy a session EGYALTALAN jelenthessen tetlenseget. Ez valasztja szet a ket olvasatot: ha igy mar suspendel, akkor a 'nem alszik el magatol' NEM a telefon tulajdonsaga volt, hanem a bekapcsolt kijelzoe; ha igy sem, akkor marad a masodik ag (elsult a policy es valami visszautasitotta), es az inhibitor-lista nevezi meg. ☠️ A bl_power=4 mar bevalt kar a sleep-night.sh-ban, tehat nem uj muszer.
        · A PREMISSZAJA MEGDOLT 2026-08-31. A 'bekapcsolt kijelzo miatt nem jelent tetlenseget a session' feltevest a meres cafolta: a wayland session (c1, greetd) IdleHint=yes volt 14:52 ota, jovval az ablak elott, es a seat0 is yes. A kijelzo elsotetitese tehat semmit nem valtoztatna. Helyette a 13-as tetel.
  [x] 13. ★ R1b UJRANYITVA: BETOLTODOTT-E EGYALTALAN A POLICY? A 2026-08-30-i nulla harom olvasatabol kettot kizartam (minden szamito session tetlent jelentett; nulla 'block'-modu sleep-inhibitor, mind csak 'delay'), es a 'Class=manager' session NEM szamit (systemd SESSION_CLASS_CAN_IDLE csak USER/USER_EARLY/*_LIGHT/GREETER). ⇒ Marad a harmadik: a /run-os logind drop-in talan sosem lepett eletbe. ☠️ A muszer eddig ROSSZ ESZKOZZEL kerdezett: 'systemctl show systemd-logind -p IdleAction' a UNIT tulajdonsagat adja es URESET ad vissza; a helyes tanu a BUS-tulajdonsag: busctl get-property org.freedesktop.login1 /org/freedesktop/login1 org.freedesktop.login1.Manager IdleAction IdleActionUSec IdleHint. Ugyanez az ures string utasitotta vissza a STEP 0 inditasat 18:24-kor (35 perc vesztes) - a kapu mar javitva (a drop-in fajl letezeset nezi).
        · ★ MEGVAN 2026-08-31: A DROP-IN BETOLTODIK. busctl-lel merve: IdleAction 'ignore'->'suspend', IdleActionUSec 1800000000->60000000, majd vissza. Ugyanekkor a systemctl show -p IdleAction MINDHAROM allapotban URES => a regi muszer bizonyitottan vak. ⇒ A HARMADIK AG IS KIZARVA: mindharom elore rogzitett magyarazat (session nem tetlen / block-inhibitor / policy nem toltodott be) MERESSEL cafolva, es a nulla suspend MEGMAGYARAZATLAN. ☠️ A manager-szintu IdleHint most 'false'-ot mutat, de EZ NEM OLVASHATO az R1b-re: a sajat ssh-sessionom Class=user, tehat beleszamit - ugyanaz a szennyezes, mint tegnap. A hianyzo tanu: a manager IdleHint AZ ABLAK ALATT, ssh nelkul.
  […] 14. R1b-3: MINTAVETELEZI-E A TELEFON SAJAT MAGANAK a manager IdleHint-et? Egy on-device unit 30 s-onkent irja fajlba a 'busctl get-property ... Manager IdleHint IdleSinceHint IdleActionUSec' hármast, mikozben az IdleAction=suspend drop-in el, es ssh NINCS nyitva. Ez az EGYETLEN modja megtudni, tetlennek latta-e magat a rendszer az ablak alatt - mert minden interaktiv olvasas maga teszi nem-tetlenne. ☠️ Elore rogzitve: ha IdleHint vegig false MARAD ssh nelkul is, akkor valami MASIK session vagy egy idle-inhibitor tartja ebren, es a systemd-inhibitor lista WHAT=idle sorat kell keresni (a korabbi listakban egy sem volt). Ha IdleHint true LESZ es megsem suspendel, akkor a hiba a logind idle-action idozitojeben van, nem a feltetelekben.
        · az MM-stopped duty-ablak utan; nem futhat vele egyszerre (mindketto a telefon nyugalmat igenyli)
  […] 15. ★ D2a — MI HIANYZIK A TERVBOL: nincs egyetlen tetel sem, ami MEGVALTOZTATNA valamit a modemen es megmerne a hatasat. A D2 csak MEGNEVEZ. Ez tudatos: a 'mit csinaljunk' nem tervezheto, amig a 'mi tortenik' nincs meg - de a hianyt ki kell mondani, nehogy ugy tunjon, a terv a celig vezet. Amint a D2 megnevezi a mechanizmust (DRX/paging/RRC), ide jon az elso A/B, ami ténylegesen csokkenti a dutyt.
        · a D2 eredmenyere var - amig a mechanizmus nincs megnevezve, nincs mit A/B-zni. Ez a tetel maga a hiany rogzitese, nem vegrehajthato munka
  […] 16. ★ MAINLINE-MODOSITAS: smp2p sleepstate port (ADSP, remote-pid 2). A vendor 4.9 smp2p_sleepstate.c-t tukrozo platform driver + DT-bejegyzes az smp2p-adsp ala. Szemantika a vendor forrasbol MERVE: bit 12 ('PROC_AWAKE_ID'), 1 = AP ebren, 0 = AP lemegy; PM_SUSPEND_PREPARE-kor torol + 10 ms varakozas ('Tuned based on SMP2P latencies'), PM_POST_SUSPEND-kor visszaallit; notifier priority INT_MAX; probe 1-re allitja. ☠️ Mainline idiomaval: qcom,smem-state (a smp2p.c-nk #qcom,smem-state-cells-t ad), NEM a downstream gpio-idiommal. ☠️☠️ EZ NEM A MODEMNEK SZOL: a vendor DT egyetlen sleepstate bejegyzese remote-pid 2 = ADSP; a modem pid 1. Tehat a downstream SEM szol a modemnek ⇒ ez NEM magyarazhatja a 34,8%-os MPSS dutyt. Elore rogzitett olvasat a leadben (leads/smp2p-sleepstate-missing.md): ha az MPSS duty valtozik, ELOSZOR a muszert kell gyanusitani. Kategoria: a valtozas a 'power' kategoriaba esik - de a fork branch-modellje szerint ELOBB el kell donteni, van-e wip/<base>/power ag, mert a CLAUDE.md tablazataban NINCS ilyen kategoria.
        · az mmdutyab utan; es elotte el kell donteni a branch-kategoriat (a CLAUDE.md tablazataban nincs 'power')
  [-] 17. ★★★ UJ FRONT: MIT CSINAL A MODEMMANAGER, AMITOL A MODEM 35%-BAN EBREN VAN? Merve: MM-stopped MPSS awake 5,1% (2026-08-31), MM-running 34,8% (korabbi). Az orakulum 5-7%-ot er el a SAJAT stackjevel futva ⇒ nem 'attach-ideju konfiguracio', hanem a mi daemonunk. ☠️ KONFUNDALO, amit eloszor ki kell zarni: az MM 11 oraja allt a meres elott, tehat az 5,1% lehet a leallitas VAGY a hosszu bekenhagyas kovetkezmenye (ld. duty-vs-uptime.sh). A/B UGYANABBAN A BOOTBAN: MM vissza -> 600 s ablak -> MM le -> 600 s ablak. Csak ha az A/B megerositi, jon a kovetkezo kerdes: melyik MM-tevekenyseg (polling? indications? signal-query?).
        · ☠️ VISSZAVONVA 2026-08-31 06:12, A/B-vel: a ModemManager NEM az ok. A-B-A' egy bootban: MM-stopped 5,1% / MM-RUNNING (registered, LTE, attached, vodafone HU) 4,9% / MM-stopped 4,9%. LAPOS. Jo, hogy lefutott, mielott megirtam volna.
  […] 18. ★ UJRANYITVA: MI VISZI A ~41 mA-t s2idle-ben? A 48 mA-es padlobol a modem (ha ~5% duty) csak ~7 mA. Gyanusitottak: az USB-link (11. tetel, most mar KRITIKUS), DDR self-refresh, PMIC, eMMC-sin, es barmi, amit a rail-census 'aktivra szavaz es sosem alvasra'. ☠️ Ez ujranyitja az X-palyat, amit 2026-08-30-an lezartam - de MAS kerdessel: nem 'parkolja-e az APSS a XO-t' (az negativ), hanem 'mi fogyaszt 41 mA-t, mikozben az AP s2idle-ben van'.
        · osszeolvadt a 11-essel: az USB-link bearazasa, ami ejszaka-hosszu kar + a felhasznalo dontese a telefon elerhetosegerol
  [x] 19. ★★★★★ A STABIL KETOLDALI DIFFERENCIAL AZ LPASS, NEM AZ MPSS. Merve: pmOS LPASS awake 100,0% (2026-08-28 ES 2026-08-31 mindharom laban), orakulum LPASS 2,8-3,0% (ot UT-capture 2026-08-28). ⇒ a 100% NEM muszer-mutermek: ugyanaz a szamlalo az orakulumon 3%-ot ad. Kozben az MPSS duty NEM allando: 34,8% (08-28) -> 4,9% (08-31), ugyanazon a stacken. ⇒ A NAPOK OTA A D-PALYAT HAJTO 34,8% NEM REPRODUKALODIK; az adat 3 napja ott ult a captures/2026-08-28_modem-window-both/ fajlokban. Kovetkezo: (a) mennyibe kerul mA-ben az ebren tartott LPASS, (b) ebren marad-e s2idle ALATT is - mert akkor o a ~41 mA fo gyanusitottja, (c) a 16. tetel (smp2p sleepstate az ADSP-nek) ezzel sokkal erosebb lett.  ⟨capture:docs/power/bringup/captures/2026-08-31_xo-dur-semantics⟩
        · LEZARVA NEGATIVAN: a tetel alapallitasa (LPASS 100% ebren = a stabil ketoldali differencial) MUSZER-MUTERMEK volt, forditott jellel. Az LPASS ALSZIK. (a) targytalan - nincs mit bearazni; (b) targytalan - nem is ebren van idle-ben; (c) a 16. tetel NEM lett erosebb ettol, mert az ADSP alszik. A tetel helyebe a 24/25 (PRONTO) lep.
  [x] 20. ☠️ MODSZERTANI: MIERT NEM VETTUK ESZRE 3 NAPIG? Az LPASS 100% ott volt a 2026-08-28-i capture-okben, a modem-window-fit.py ki is irja minden futasnal. A figyelem az MPSS-en volt, mert a kerdes 'a modem' volt. Elore rogzitendo szokas: MINDEN modem-window futasnal a NEGY master mindegyiket olvasni, es a nem-vart 100%-ot ugyanugy leletnek venni, mint a vart szamot.
        · SKILLBE EMELVE VALOBAN (004d681): 'a multi-channel instrument read as a single-channel one hides its own answer' - minden oszlop olvasando minden futasnal, es egy telitett ertek (0%, 100%, nem mozdulo szamlalo) eseten EXPLICIT dontes kell, lelet-e vagy mutermek; a referencia-oldal ugyanazzal a muszerrel egy grepben eldonti.
  [x] 21. ☠️ HIANY: A NEGY HOOK SEHOL NINCS DOKUMENTALVA - sem a ~/.claude/CLAUDE.md-ben, sem a skills-repo README-jeben. Fut: autonomy.cjs (terv + Stop-blokk), measurement-watch.cjs (meres+figyelo parositas), risky-target.cjs, precompact-status.cjs. Kettő erdemben alakitja a munkamenetet, es egy uj session csak akkor tud roluk, ha veletlenul elsulnek. Megirandó: mit csinal, mikor sul el, milyen parancsai vannak (a wait-hianya ma epp ebbol lett hiba).
        · MEGIRVA 2026-08-31 (9da774a): a skills-repo README-jebe uj szakasz - autonomy.cjs teljes parancslistaval es a wait kulon kifejtve (mit kerul, ha kimarad: az emlekezteto ismetlodik, es a legkonnyebb kielegiteni ugy, hogy talalok magamnak tennivalot = a merest varo eszkoz piszkalasa), measurement-watch.cjs mindket orevel es a ket elvesztett futassal, plusz a ket nem-plugin hook es a ketpeldanyos szetcsuszas figyelmeztetese. A ~/.claude/CLAUDE.md-be NEM irtam: az privat es minden projektre szol, ez repo-specifikus.
  [x] 22. ☠️ AUTO-COMPACT ADATVESZTÉS: az autonomy hook átírása öt ponton — SessionStart (a compact utáni session prompt nélkül kapja vissza a tervet), PreCompact (a resume-blokk lemezre ürítése), generált resume-blokk a STATUS.md tetején marker-párok között, kötelező+ellenőrzött bizonyíték a done/measured mellé, és measured/retracted mint külön rekordtípus, plusz Stop-avultságkapu a watch-könyvtárakra  ⟨commit:df1ecf3⟩
        · Mind az öt kész és known-positive ellen validálva: hat elutasításnak KELL elutasítania (nincs bizonyíték / nemlétező fájl / feloldhatatlan sha / EOF utáni sor / hiányzó tanú / üres unverifiable-indok), három elfogadás átment, a marker-blokk ismételt flush-ra idempotens, az avultságkapu új capture-könyvtárra tüzel és a rekordra tisztul, az anti-spin négy változatlan kör után elenged. ☠️ Egy önmagát tüzelő kapu is kiderült és javítva: a hook írja a STATUS.md-t, attól lett piszkos a repo, attól tüzelt a kapu — a saját fájlját most kizárja. A pmaports oldal c6fab06.
  [x] 23. ★★★ MINDEN EDDIGI DUTY-SZAM UJRAOLVASASA A JAVITOTT MUSZERREL (87be062 + 474ff79). Ket fuggetlen parser-hiba dolt ki egy oran belul, es MINDKETTO ugyanazt a hamis 'awake 100,0%'-ot allitotta elo. Minden duty-szam, ami valaha ebbol a scriptbol jott - beleertve a D-palyat hajto MPSS 34,8%-ot es az orakulum 5-7%-at - a hibas verzioval keszult. A munka: a javitott fit ujrafuttatasa MINDEN meglevo capture-on, es egy tabla arrol, melyik korabbi kovetkeztetes eli tul. ☠️ Elore rogzitve: ha a 34,8% is elszall, a teljes D-palya alapja szunik meg; ha megall, akkor megerosodik, mert most mar hitelesitett muszertol jon. Eszkoz nem kell hozza, a telefon nyugalmat nem zavarja  ⟨commit:b3a5295⟩
  [x] 24. ★ PRONTO (WCNSS/WiFi) mint a ~41 mA gyanusitottja: 16,7-19,1% ebren pmOS-en, 21,3-23,1% az orakulumon. NEM a ket rendszer kulonbsege, de a padlo szempontjabol gazdatlan fogyaszto. Elore rogzitendo: a WiFi le van-e egyaltalan kapcsolva a padlo-meres alatt? A step-0 ejszaka kapui (MM stopped, bl_power=4, Discharging) a WiFi-t NEM ZARTAK KI - ha a wlan fent volt, a 48 mA tartalmazza a WCNSS koltseget is. Eloszor a NAPLOBOL kell megnezni, nem uj merest inditani.  ⟨commit:e303a6a⟩
        · MEGVAN, NAPLOBOL: wlan0 vegig fent volt (2803 wlan-sor az ablakban, DHCP-lease 04:37:03, 192.168.100.17) ⇒ a 48 +/- 5 mA-es padlo TARTALMAZZA a WiFi-t; a step-0 harom kapuja kozul egyik sem erinti. Az elore rogzitett szabaly ('eloszor a naplobol, ne uj meressel') betartva.
  [x] 25. PRONTO s2idle ALATT: ebren marad-e? A javitott muszer szerint a PRONTO 16,7-26,7%-ban ebren van MINDKET rendszeren, de kizarolag EBREN-ablakokban merve. A 18. tetel (mi viszi a ~41 mA-t s2idle-ben) szempontjabol pontosan a masik allapot szamit, es azt soha senki nem merte. ☠️ Elore rogzitve: ha a PRONTO s2idle alatt is ~20%-ban ebren van, uj nevesitett fogyaszto a padlon; ha lemegy, akkor a PRONTO teljesen kiesik a 18-bol es marad az USB-link (11. tetel). ☠️☠️ A tartomanyok ATFEDNEK a ket rendszer kozott - ez NEM differencial, tehat onmagaban nem magyarazza a pmOS-UT kulonbseget, csak a padlo egy tetelet  ⟨capture:docs/power/bringup/captures/2026-08-30_xo-across-suspend⟩
        · MAR MEG VOLT MERVE, prior-art-tal talalva, ablak elkoltese nelkul: egy VALODI 601 s-os suspendben a PRONTO XO-off deltaja 613,9 s = a 626 s-os ablak 98,1%-a ⇒ a WiFi-mag alvas alatt LEENGEDI a kristalyt, tehat KIESIK a 18. tetelbol. Bonusz: ugyanez a sor rogziti a tick-rate-et 19,2 MHz-en ket szazalekon belul, es bizonyitja, hogy a szamlalok a suspend ALATT is haladnak
  [x] 26. ★★★ A HIANYZO CELLA: MENNYIRE EBREN AZ MPSS EGY VALODI SUSPEND ALATT, MM NELKUL? A 48 mA-es padlo MM-STOPPED-dal lett merve, es az egyetlen suspend-alatti MPSS-szamunk (45%) MM-RUNNING-gal keszult. A 'a padlo a modem' allitast pont azert vontam vissza, mert az ebren-ablakos 5,1%-ot alkalmaztam az alvo telefonra - de az ebren-ablak bizonyitottan MAS REGIME. Ez a cella donti el, hogy a visszavonas maga volt-e elhamarkodott. Eszkoz: rpm-xo-snapshot.sh elotte/utana egy 600 s-os rtcwake korul, MM leallitva (mar az). Nem kell hozza suloallapot, sem kisules, sem a felhasznalo dontese. ☠️ ssh NEM lehet nyitva az ablak alatt. Elore rogzitve: ha ~45% marad MM nelkul is, a modem a padlo fo tetele es a visszavonas tul korai volt; ha ~5%, a visszavonas all es a ~41 mA tovabbra is gazdatlan  ⟨capture:docs/power/bringup/captures/2026-08-31_mpss-across-suspend-nomm⟩
        · A MASODIK AG JOTT BE: MPSS 5,0% ebren egy tiszta, 602,2 s-os, riasztassal vegzodo suspendben (suspend_stats 72->73, fail=0, wakeup_irq=56=RTC), MM leallitva. Ugyanaz, mint az ebren-ablakban (4,9-5,1%) ⇒ MM nelkul a duty NEM regime-fuggo ⇒ A REGGELI VISSZAVONAS ALL, a modem ~7 mA, es a ~41 mA tovabbra is gazdatlan
```
