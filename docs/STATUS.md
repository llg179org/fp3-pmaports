...
## A futás rögzített tényei — 2026-08-31 … 2026-09-03

> ☠️ **Ez a szakasz már NEM generált, és ez a lényeg.** 2026-09-03-ig a
> nyugalmazott `autonomy.cjs` írta újra minden terv-szerkesztéskor, és a fejléce
> azt mondta magáról, hogy „edit the plan, not this block". A hook leállítása
> után a terv helyére *„No autonomous run is active."* került, a tények viszont
> bent maradtak — vagyis egy blokk, ami **magáról állít valamit, ami már nem
> igaz**, és amit senki nem ír újra. Pontosan az a rothadás, amit ez a projekt
> kétszer is megfizetett. Ezért ki lett bontva a keretéből: mostantól közönséges,
> kézzel karbantartott szakasz.
>
> A hozzájuk tartozó **feladatlista** két helyen él: a nyitottak a
> [`TODO.md`](TODO.md) `FP3-QUEUE` szakaszában, a lezárt 104 tétel a
> [`TODO-DONE.md`](TODO-DONE.md) végén.
>
> **77 mért állítás + 23 visszavonás.** A visszavontak nem törlődnek: egy törölt
> cáfolat újra felfedezhető.
...
MEASURED and standing:
  ...
  · Elerhetoseg IMS=off mellett: 14/14 kezbesites (4 reggeli + 10 a ringlogbol, 10:31-tol 18:57-ig, ~orankent). A tiz naplozott hivas eszkozoldali kesleltetese 375,5 +- 34,7 ms (terjedelem 342-463) - SZOROSABB, mint a reggeli kezi merese (385 +- 92), mert a naplozo ugyanazt a ket journal-sort hasonlitja ossze minden alkalommal. MIND A TIZ hivas utan a sav gsm/gsm-900-extended => a CSFB minden hivasnal megtortent. Also korlat 0,05^(1/14) = 0,806. scope: egy nap, egy cella-kornyezet, orankenti kozok - a 8 oras tetlenseg-sarok NINCS mintavetelezve. ☠️ UNVERIFIED
      witness: unverifiable:/var/log/fp3/ringlog.tsv-a-telefonon
  · A HALOZAT AD IMS-t ennek a SIM-nek: a hurok-capture 22 bearer-elfogadasaban a PCO ket P-CSCF cimet hoz (10.149.x.x x22, 10.150.x.x x21), a 'held' capture ugyanezt, es az IMS=off kontroll - nulla elfogadas - NULLA cimet. Tehat az imsd-tartaleknak VAN hova regisztralnia; a halozati fel nem akadaly. scope: vodafone HU, 2026-09-02, egy SIM. ☠️ NEM jelenti, hogy az uzemelteto beengedne EZT a keszuléket - az IMS-regisztracio gyakran keszulek-policyhoz kotott, tehat az ut KET kapun all es ez csak az elso halozati fele.
      witness: docs/power/bringup/leads/volte-is-provisioned.md
  · Az ejszakai replikacio EGY labat futtatott haromból, es az a lab is hasznalhatatlan. KET fuggetlen ok: (1) a night-run.sh lepes-aritmetikaja szerkezetileg csak egy labat engedett - a step+2 iras a reboot_now ALATT allt, ami nem ter vissza, tehat az allapot a paros advance-first erteken maradt es a lab-ag csak paratlan lepesen fut; JAVITVA es szekvencia-teszttel bizonyitva, a teszt a REGI sorrenddel pontosan azt a nyomot adja, amit a telefon csinalt. (2) A lab1 49,4 mA-t olvasott 11 s median alvassal a 90 s-os ebreszto ellen, es NetworkManager Script Dispatcher indult bele => a tegnapi dobasi szabaly kivette. Mindket OCV-vegpont a plafonjaba utkozott leules nelkul (-0,91 / -0,78 mV/perc). ★ A vege-figyelo MUKODOTT: 0 ssh-belepes a labban - a 19:22-es javitas beigazolodott.
      witness: commit:6d22b85
  · A 121. FOPROBA MIND A NEGY ZOLD FELTETELT TELJESITI: (1) 3 lab, 3 KULON booton; (2) EGY 'NIGHT COMPLETE', service letiltva; (3) a slope most SZAMOT ad (-4.01) a javitas utan, telefonon ellenorizve; (4) ☠️ a labakon BELUL NULLA wlan0 DHCP-esemeny - a naplozott esemenyek pontosan a lab-indulaskori nemitas (nmcli managed no) es a lab utani visszakapcsolas. ☠️ Az elso olvasatom ('2/1/5 esemeny a labakban') KET rossz muszerbol jott: a busybox 'date -d +30 seconds' ervenytelen, es a journalctl --since ablakot nem zartam le a lab VEGENEL. A rehearsal.conf drop-in TOROLVE, tehat este nem 3 perces labakat mer.
      witness: capture:docs/power/bringup/captures/2026-09-02_night-replication
  · ★ A CERTIFIED HANDSET DOES GET VoLTE ON THIS NETWORK — the second gate is NOT closed. Paired reading on the daily factory-Android FP3 (dual SIM, one handset, one place, minutes apart): BEFORE the call BOTH SIMs read 4G; DURING the call SIM1 falls to GSM/EDGE (CSFB) while SIM2 STAYS on 4G. Same radio, same coverage, same moment ⇒ the variable is the SUBSCRIPTION, not the device and not the cell. This REFUTES the premise the #112 pre-filter was braced for ("if a certified handset also falls back, the imsd path is pointless on this network"): VoLTE is carried here. RESOLVED 2026-09-03 (132): BOTH SIMs are vodafone HU, replicated over several alternating calls — the 4G one is the PRIVATE subscription, the GSM/EDGE one the CORPORATE. So vodafone HU carries VoLTE and the corporate plan simply does not have it enabled: the variable is the TARIFF, not the network, not the cell, not the device. Our dev FP3 reads vodafone HU, MCC/MNC 21670, registered on LTE, 'ue mode of operation: csps-1' (combined CS/PS attach). ☠️ Do NOT read that csps-1 as the network's verdict on our subscription — the UE's own voice-domain preference feeds it, and our pmOS has no working IMS stack, so the device is part of why the attach is combined. ☠️ The caller's RAT and ours are INDEPENDENT: the gsm-900-extended rows in ringlog.tsv are OUR radio falling back, not the caller's. ☠️☠️ AND THE PAIR DOES NOT TRANSFER TO US: there are THREE SIMs in play, not two. The daily handset carries the corporate and the private card; the dev phone carries a THIRD card, on neither of those two plans. So this reading proves only that vodafone HU carries VoLTE on some subscriptions — it says nothing about the dev one, which has to be characterised on its own (133). An earlier version of this entry, and the commit that added it, framed the open question as "which of the two plans is ours"; that framing was wrong.
      witness: unverifiable:person read Settings -> About phone -> SIM status -> 'Mobile network type' on the daily handset, paired before-vs-during, 2026-09-03
  · ★★★★★ THE DEVICE-POLICY GATE IS OPEN — IMS REGISTERS ON THIS PHONE, THIS SIM. On the UT oracle slot (2026-09-03 12:44, same device, same IMEI, same card, nothing moved and nothing reconfigured) ofono on the vendor RIL reports IpMultimediaSystem Registered=1, Registration=auto, VoiceCapable=1, SmsCapable=1, on LTE, operator 216/70. Corroborated in the same capture by the vendor's own daemons: imsqmidaemon and imsdatadaemon both 'running', vendor.ims.QMI_DAEMON_STATUS=1, DATA_DAEMON_STATUS=1. Read twice, independently, in two separate dumps. ⇒ the operator ADMITS this IMEI to IMS registration, and VoLTE is enabled on THIS subscription (the dev phone's third card, on neither of the daily handset's two plans). Together with the network half already measured (two P-CSCF addresses + IM CN Subsystem Signalling Flag in 21/21 and 18/18 closed PCO walks), BOTH gates the imsd path stood on are now open. ☠️ It does NOT say our pmOS stack can register - it has no IMS implementation at all; this measures the subscription and the operator's policy toward this IMEI, which is exactly what our own stack cannot measure, and is why the oracle slot was the instrument. ☠️ The operator's name on the air is 'One HU', not 'Vodafone HU' - our pmOS prints the older name from its own database; 21670 is the identifier to trust. The whole oracle window cost 6.5 minutes (12:42:39 pmOS down -> 12:49:08 pmOS back).
      witness: capture:docs/power/bringup/captures/2026-09-03_ut-oracle-ims
☠️ RETRACTED — do not rebuild on these:
  ...
  · a P-CSCF cimek az ACTIVATE DEFAULT EPS BEARER CONTEXT ACCEPT (ESM 0xC2) PCO-jaban jottek vissza, 10.149.x.x x22 es 10.150.x.x x21, a 22 bearer-elfogadas ellen
      fell because: hossz-validalt TLV-seta: az ACCEPT-ben NINCS PCO IE (0 / 22). A halozat a letolto REQUEST-ben (0xC1) adja vissza; a regi szkenner MINDEN ESM-uzenetet bajt-mintaval pasztazott, es a 22 az elfogadasok szama volt, nem a cimeke. Helyes: 21/21 es 18/18 zart seta, mindket cim pontosan egyszer REQUEST-enkent, mellette DNS 80.244.x.x es az IM CN Subsystem Signalling Flag. A KOVETKEZTETES VALTOZATLAN. (88f9ea1)
```

```
OUTSIDE REVIEW: last 0.2 h ago, 0 result(s) recorded since.
  · Fable #18 (hajcsar-atterveztes). ELLENORIZVE, A SAJAT SZAMAIM KOZUL HAROM MEGDOLT: (1) 124 tetel, nem 122 (86+18+18+2) - sajat osszeadasi hiba; (2) ☠️ A FO DIAGNOZISOM MEGDOLT: NEM igaz, hogy '12 waiting proza-parkolas after helyett' - MERVE 9/18-nak MAR VAN after-je (19,30,31,41,54,55,64,85,118), es szuk regexszel 7 olvasodik parkolonak, abbol csak KETTONEK (53., 82.) nincs after-je. A gráf megvan; ☠️ A VALODI HIBA A RENDERERBEN: a blockedItems CSAK todo/doing statuszt nez (588. sor), tehat egy 'waiting' tetel after-rel SOHA nem eri el a tomor BLOCKED szakaszt, hanem a bobeszedu WAITING-be esik. Ez ~1 soros javitas, nem atterveztes. (3) a 112. NOTE 588 karakter, nem ~900. Tovabba: renderHuman HAROM helyrol hivodik (772, 826, 1533), nem kettobol; es a 'nincs szinkron a TODO-val' pontosabban 'egyiranyu szinkron a STATUS.md-vel (flushStatus), a TODO.md-t sosem OLVASSA'. AMIT A BIRALAT HOZZATESZ: (A) a tenyek listaja mehet a repoba, DE a retracted funkcioja a SessionStart-i visszaolvasas, nem a tarolas - egy fajl amit senki nem olvas fel, nem letezik; a kihagyott, nem-lista funkciok: az 'unrecorded-result' kapu (3x sult el, ez a compaction-veszteseg EGYETLEN aktiv vedelme es pont a C-felelmem ellenszere), a humanAt ora, es az anti-spin koltsegvetes. (B) a TODO.md 3713 sor, NULLA checkbox, datalt naplo ⇒ a 'nulla lista a hookban' a gyakorlatban 'egy UJ kis lista a repoban + parser a hookban' - ezt KI KELL MONDANI a felhasznalonak, nem elcsusztatni. (D) NE suketitsuk uresjaratra (az ugyanaz a hiba, mint a 09-01-i elmaradt biralat), hanem dontesi ponthoz kotni + az ora AKTIV orakat szamoljon. ☠️ A BIRALAT LEGFONTOSABB MONDATA: ami ma valoban hianyzik es sem a mai, sem a tervezett hook nem oldja meg, az a 19:00-s INDITAS IDOZITESE es a KABEL-dontes - az veszi meg vagy veszti el a ma ejszakat, nem a hook-atepites. A gateLog: 9 review-due 37 kapubol; a mai 07:36-os az elso, ami ures actionable mellett sult el.
```
...
reachable at `192.168.x.x` from ~05:35 without a cable. Plugging in earlier
...
| **data context** | **none** — `rmnet_ipa0` DOWN, 0 bytes, no bearers | **active** — `context1`, `rmnet_data2`, 10.124.x.x, Vodafone APN |
| **MPSS awake** | **34.8 %** | **6.1 %** |
