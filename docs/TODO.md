...
- [@] 181. Let the phone actually suspend when idle (logind IdleAction), leave it overnight, then take the FIRST incoming call after that long idle - the unsampled dangerous corner of #63 --why ☠️ #63 says in its own note that the dangerous corner is UNSAMPLED and that hourly calls are the wrong instrument for it, not too few of it: they never test a phone that has been idle for hours. Measured 2026-09-06 05:50 why the corner cannot be reached by simply waiting: after 5 h 03 with the screen off, /sys/power/suspend_stats/success = 0 - the phone had not suspended ONCE. logind's IdleAction is unset in logind.conf and logind.conf.d, so the default 'ignore' applies and nothing ever puts it down; systemd-inhibit shows only delay-mode sleep inhibitors (ModemManager, NetworkManager, rtkit, UPower, gsd) and no block-mode one. This matches what docs/power/ already records - pmOS never suspends by itself here - while s2idle itself works when triggered (6/6 suspends, full duration). So the corner needs a CONFIGURATION CHANGE, not patience. Procedure: (1) record the before-state (logind.conf + any drop-in, systemd-inhibit --list, suspend_stats) into the capture BEFORE changing anything - it is the way back; (2) set IdleAction=suspend with an IdleActionSec, as a drop-in so the change is one file to delete; (3) confirm it actually suspends (suspend_stats/success climbs, and the phone stops answering ssh); (4) leave it overnight UNTOUCHED; (5) the operator places ONE incoming call in the morning and reports whether it rang and how long it took. ☠️ DO NOT SSH IN BEFORE THE CALL - a login wakes the phone and destroys the very state being sampled; docs/power/ records a replication night lost exactly that way, which is why fp3-ssh carries the fp3-measure lock. ☠️ It conflicts with #158 (night replication): both want the night, so choose one. ☠️ Ordered AFTER the touch work by the operator 2026-09-06: an unattended night is worth little while a -EIO storm can still kill the touch IRQ --lane phone
      after: 179
      they-do: LEAVE THE PHONE ALONE tonight, then in the morning place ONE incoming call to it and say whether it rang and roughly how long it took. Nothing else is needed. Steps 1-3 are done: IdleAction=suspend is armed as a single deletable drop-in, the before-state is in captures/2026-09-06_181-idle-suspend-night/, and ~/.fp3-measure.lock refuses logins until 08:00. ☠️ DO NOT let anything ssh in before that call - measured 2026-09-06: an ssh session reports IdleHint=no, which holds the logind manager's hint false, and the phone then CANNOT suspend at all while it is connected. ☠️ Still unproven when this was written: that it actually suspends. Check /sys/power/suspend_stats/success first thing - if it is still 0 the night measured nothing and the drop-in needs a different IdleActionSec or a blocking inhibitor hunt. The way back is deleting /etc/systemd/logind.conf.d/99-fp3-idle-suspend.conf and restarting systemd-logind
      when: the morning after an untouched night<!-- FP3-QUEUE:END -->
