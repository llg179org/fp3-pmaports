...
- [@] 163. Put the dev card - now proven VoLTE-capable - back in the FP3 and call it
      lane: phone
...
- [ ] 164. Read the IMS registration fields the ofono plugin throws away - pAssociatedUris says whether IMS really registered
      lane: phone
      why: ☠️ The modem returns QtiRadioRegInfo {state, error_code, error_message, radio_tech, uri} for getImsRegistrationState, and ofono-binder-plugin-ext-qti reads ONLY state (qti_ims_reg_status_response: 'state = info->state'). The discarded fields answer the question the whole VoLTE thread is stuck on: pAssociatedUris is the P-Associated-URI from the SIP REGISTER 200 OK, so a populated value means the device genuinely completed IMS registration with the operator core, and an empty one means it did not - which separates 'the subscription has no VoLTE' from 'the stack does not finish IMS voice', two mechanisms that look identical from outside because the network's T-ADS decision collapses them. error_code/error_message give the reason when it fails, radio_tech says which RAT the registration sits on. Route: patch the plugin's DBG to print the whole struct and rebuild it (source: gitlab.com/ubports/development/core/hybris-support/ofono-binder-plugin-ext-qti, it is ~4700 lines and builds as a standalone .so), or recover the fields from the binder hexdumps ofonod already writes to the journal. ☠️ Independent of #163 and worth doing even if #163 answers first - #163 tells you WHICH side, this tells you WHY
<!-- FP3-QUEUE:END -->
