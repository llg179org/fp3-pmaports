...
- [@] 158. Night replication, third attempt: record WHAT WAKES THE AP before running another night — add a per-leg /sys/kernel/debug/wakeup_sources (or wake-reason) snapshot to night-run.sh, then re-run the three-boot replication
      ...
      until: 09-05 10:00
      they-do: ☠️ CORRECTED AGAIN 2026-09-05 16:15: the phone is on pmOS slot b running r82 (uname #83-fp3, source 3f843d05), rebooted 15:43 - two flashes happened after the previous note was written. The slot is not the blocker. Nothing arms itself: to run tonight the run must be armed on the device before 19:00. Say the word and this window arms it; the task's own three requirements still stand - record WHAT WAKES THE AP before the run, log each leg's userspace config, and remove the ModemManager debug drop-in. ☠️ It conflicts with #175, which needs the phone USED normally during the day rather than measuring, and #175 unblocks two other items
...
- [@] 175. #142/#155 FIELD CONFIRMATION: use the phone normally for a session with pauses (>= ~36 min of real tapping - the measured gaps between stalls run to 726 s, so anything shorter can come back clean while the fault is fully present), then run fp3-selftest --only touch. The mechanism is already fixed and measured on r81 (l6 keeps a voter across display-off, l10 has a consumer); this is the only thing that can speak to the operator-visible 'Failed to read input event: -110'.
      ...
      why: The mechanism is fixed and measured: with the driver bound, l6 keeps a voter across display-off (2 consumers, falling to 1 instead of to 0) and l10 has a consumer for the first time. Measured on r81; the phone now runs r82, which carries the SAME source commit 3f843d05, so the fix is unchanged and the field confirmation is equally valid there. ☠️ 142-trigger.sh is NOT the test - it unbinds the driver, and unbind releases the supplies (measured: l6 2->1, l10 1->0), so on a fixed kernel it recreates the pre-fix rail state
...
- [~] 177. Package imsd for pmOS (aarch64) and get it to REGISTER - not yet to call. Source: forgejo.catcrafts.net/Catcrafts/imsd, GPL-3.0, Makefile route (clang++ + libc++ + lld + gio-2.0 headers; libc++ 22.1.8 is packaged for Alpine edge aarch64). Then configure it from what this repo already measured: PCSCF from the two P-CSCF addresses in leads/volte-is-provisioned.md (its own P-CSCF discovery from the PDN PCO is NOT implemented, so this value must be given by hand), DEV = the IMS PDN device raised with mmcli --simple-connect apn=ims, LOCAL auto from a global IPv6 - CHECK that the IMS PDN actually gets one, it is listed as auto-detected and may not exist here. Success for this task is a completed SIP REGISTER, nothing more. ☠️ Needs no test call: registration either happens or it does not
      ...
      until: a capture of the STOCK stack's REGISTER exists to diff against - the Ubuntu Touch side registers successfully with this same card against this same core (#163), and imsd's DumpRaw was built for exactly that comparison. Same capture wall as #54 and #64<!-- FP3-QUEUE:END -->
