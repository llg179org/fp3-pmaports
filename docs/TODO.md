...
- [ ] 160. Put the DEV SIM in the daily factory-Android handset and call it — one variable, the card
      ...
      they-do: move the dev FP3 card into the daily Android handset, call that handset from another phone, and read the RAT DURING the call. ☠️ The status bar does NOT show it during a call (measured 2026-09-03) — use Settings -> About phone -> SIM status -> "Mobile network type", which updates live, or turn WiFi off on the calling handset and see whether mobile data survives the call. One word is the answer: does it stay 4G or drop to GSM/EDGE
