...
- [@] 184. Himax IDLE MODE: test whether the controller's idle scan is the mechanism behind the same-side tap outages. Write 0x17 to register 0x10007088 byte 0 (0x1F = idle on, 0x17 = off), named by the oracle's vendor driver himax_idle_mode() in ubports-fp3-kernel/drivers/input/touchscreen/hxchipset83112b/himax_ic.c. Needs the vendor register protocol (himax_register_read / himax_flash_write_burst), NOT a plain i2c write
