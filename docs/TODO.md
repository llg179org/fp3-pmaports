...
- [~] 158. Night replication, third attempt: record WHAT WAKES THE AP before running another night — add a per-leg /sys/kernel/debug/wakeup_sources (or wake-reason) snapshot to night-run.sh, then re-run the three-boot replication
      ...
      until: 09-05 10:00
      they-do: When arming the night, export FP3_TASK=158 so the completion sentinel names this task — night-run.sh now writes /var/log/fp3/completions/<task>.done on every exit path including give_up, and `queue-sync.cjs` on the host turns that into queue movement. Unset FP3_TASK writes task 0 and is reported as unattributed every run rather than annotating the wrong entry. If this night gets an evaluator task, mark it [~] with `until: completion:158` and NOT `after: 158` — see tools/COMPLETION-LOOP.md for why the second form deadlocks.
- [@] 166. Load the Hungarian carrier MBN into the modem and see whether VoLTE appears
      lane: phone
      why: ☠️ THE EXPERIMENT #165 set up, and the first one in this thread that changes something rather than measuring it. Measured 2026-09-05: the modem runs the generic ROW_Commercial config and has no Hungarian MBN loaded at all, though vodafone/commerci/hungary and dt/commerci/hungary both ship in the vendor store (134 MBNs on disk, 25 loaded). Under a generic config IMS registers but MMTEL voice is not enabled, which is exactly the measured symptom. ☠️ THIS CHANGES MODEM CONFIGURATION, not anything in our tree, so: record the before-state FIRST (the full pdc config list with statuses, so the current active config can be restored), then load and activate vodafone/commerci/hungary, then re-run the same call measurement that gave EDGE with this card in captures/2026-09-05_163-same-card-two-devices. pdc can deactivate and restore, but only against a before-state that exists. ☠️ Do it on the slot where a pdc query actually works - it does on pmOS, and on UT there is no qmicli or pdc and no QMI char device. ☠️ A negative result does not clear the MBN theory on its own: the Hungarian config sits under commerci rather than volte and may not enable VoLTE either, and operator IMEI/TAC gating would give the same symptom
      ...
      after: 167
- [ ] 167. Parse the MBN values, not just the item names - does the Hungarian config actually prefer PS voice?
      lane: upstreaming
