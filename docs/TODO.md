...
- [ ] 169. Capture an LTE attach and read the IMS-voice-over-PS bit the network sends this UE
      lane: phone
      why: ☠️ THE MEASUREMENT THAT DECIDES whether the operator refuses IMS voice to this DEVICE, which is the one device-side mechanism left untested after #163 put the fault on the device and #165/#167/#168 failed to convict the modem config or the HAL version. The ATTACH ACCEPT (EMM 0x42) and TAU ACCEPT (EMM 0x49) carry the EPS network feature support IE (0x64) whose IMS-voice-over-PS bit is the network telling THIS UE whether it may use IMS for voice. Not supported => the refusal is network-side, subscription or device policy, and no local change helps. Supported while we still CSFB => the fault is below ofono on our side. ☠️ NOT readable from what we hold: tools/diag-ota-decode.py over captures/2026-09-02_diag-ota-pmos/raw/diag.bin finds 612 log entries with exactly ONE EMM entry and no Accept - the device was already attached for the whole window. Needs a NEW capture spanning an attach (airplane mode off, or a modem restart) with QCSuper, on pmOS where the diag port is reachable. ☠️ Walk the IEs by their own lengths and make them close on the message boundary, as tools/pcscf-scan.py does - this repo has already published one wrong conclusion from a byte-scan that searched too widely. Lead page: leads/imei-tac-gating.md
<!-- FP3-QUEUE:END -->
