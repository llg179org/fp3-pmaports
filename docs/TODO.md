...
- [@] 142. Bert's regression: 0314fee3ce35 (msm8953.dtsi system-pc arm,psci-suspend-param 0x41000353 -> 0x42000353, affinity level 2) breaks his hx83112b touchscreen after resume (i2c -110/-6) on a second FP3; reproduce on ours (touch after suspend, before/after revert), and HOLD the msm8953.dtsi idle-state patch out of any series until understood
      ...
      until: ARM A DONE 2026-09-07 (5/5 off, 0/5 on) and its pre-registered criterion is UNREACHABLE by its own instrument: 142-trigger.sh unbinds the driver first and the rail fix IS that driver devm regulator vote - proven in regulator_summary, l6 has 2 consumers bound and 1 unbound. What arm A did give: the stall fell 15.07 s/-110 -> 2.05 s/-5 (the i2c-qup recovery). ONLY ARM B REMAINS and it needs a person: after a real suspend, driver bound, MINUTES of tapping (>=500 touch interrupts, the 09-04 rate was ~1 x -110 per active minute). ☠️ Blocked in practice by the phoc DSI resume bug - the screen must come back before anyone can tap it. Also still needs Bert to say WHICH commits his tree carried
