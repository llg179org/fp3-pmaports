...
- [@] 158. Night replication, third attempt: record WHAT WAKES THE AP before running another night — add a per-leg /sys/kernel/debug/wakeup_sources (or wake-reason) snapshot to night-run.sh, then re-run the three-boot replication
      ...
      until: 09-05 10:00
      they-do: Nothing arms itself: a night run must be armed on the device before 19:00. Say the word and this window arms it. The task's own three requirements stand - record WHAT WAKES THE AP before the run, log each leg's userspace config, and remove the ModemManager debug drop-in. ☠️ Names no release on purpose: this note was corrected three times in one day because flashing kept invalidating it. Ask the device: fp3-ssh 'uname -v; cat /usr/share/kernel/fp3/fp3-commit; uptime'. ☠️ It conflicts with #179, which needs the phone USED for 36 active minutes rather than measuring, and #179 unblocks #178, #166 and #140 - so choose one, do not arm both
...
- [@] 182. ModemManager loses the modem across suspend/resume: on wake it rebuilds its device list, finds no QMI port and gives up without retrying - 'couldn't create modem for device qcom-soc: Unsupported device: at least a QMI port is required'. The phone then has NO cellular service until ModemManager is restarted, while the modem firmware is fine (three remoteprocs running, qmicli --dms-get-model answers). Measured 2026-09-06 23:23: suspended 23:04:02, woken by an incoming call 23:23:38, and the call could not ring because there was no modem. Fix candidates, cheapest first: (1) find out WHY the qrtr QMI port is not there at probe time on resume - is the qrtr node re-created late, or does MM probe before qrtr comes back; (2) if it is a race, a systemd unit After=suspend.target that restarts ModemManager is a workaround, NOT a fix, and it costs the first-call latency #181 wants to measure; (3) upstream: MM should retry the probe rather than concluding unsupported. ☠️ #181 CANNOT run until this is fixed - its morning sample would be a guaranteed 'did not ring' read as a reachability result --lane phone --prio 15
      ...
      they-do: ONE incoming call at the end of a ~12 minute suspend, to run the failing arm while a QRTR service dump is taken at +0 s on resume. Say the word and this window arms it; the phone is restored automatically afterwards. ☠️ It is an INTERACTION, not a duration: measured overnight, 24 s + call survived, 665 s + RTC survived, and 623 s + call and 1176 s + call both LOST the modem. Only a long suspend ended by an INCOMING CALL loses it, so an RTC-woken run - which needs nobody - cannot reproduce it and three of them already did not. What is still unmeasured is how late the QMI port is in the failing arm, and that is the number a retry in ModemManager needs
