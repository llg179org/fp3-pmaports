...
- [~] 166. Load the Hungarian carrier MBN into the modem and see whether VoLTE appears
      lane: phone
      why: ☠️ REOPENED 2026-09-05 on pmOS r81, where the blocker does not apply: the modem already HOLDS 25 software configs and one of them is Global-VoLTE-Vodafone, so the question is an ACTIVATE (a different qmicli code path) and not the --pdc-load-config that segfaults. pmOS also offers real IMS instruments UT did not: --imsa-get-ims-registration-status, --imsa-get-ims-services-status, --ims-get-ims-services-enabled-setting, --imsp-get-enabler-state, --voice-get-config. Restore is unchanged: activate software,5C:F9:CA:DA:5C:35:85:17:BA:3B:B8:88:D0:34:2B:79:BD:5F:AD:ED (ROW_Commercial)
