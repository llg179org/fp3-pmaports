...
- [@] 158. Night replication, third attempt: record WHAT WAKES THE AP before running another night — add a per-leg /sys/kernel/debug/wakeup_sources (or wake-reason) snapshot to night-run.sh, then re-run the three-boot replication
      ...
      until: 09-05 10:00
      they-do: ☠️ CORRECTED 2026-09-05 14:15: the old note said the phone has been on UT since 04:47. That is stale - it is on pmOS slot b now, running r81 (3f843d05), rebooted 13:25, so the slot is no longer the blocker. Nothing arms itself: if this is to run tonight the run has to be armed on the device before 19:00. Say the word and this window arms it, and the three things the task already requires still stand - record WHAT WAKES THE AP before the run, log each leg's userspace config, and remove the ModemManager debug drop-in. ☠️ A night run also conflicts with #175: the touchscreen field confirmation needs the phone used normally during the day, not measuring
