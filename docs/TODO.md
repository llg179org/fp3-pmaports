...
- [~] 166. Load the Hungarian carrier MBN into the modem and see whether VoLTE appears
      lane: phone
      why: ☠️ ATTEMPTED 2026-09-05 UNDER THE OPERATOR'S AUTHORISATION AND BLOCKED BY A TOOL CRASH, not by the modem. Before-state recorded first (captures/2026-09-05_mbn-hungary-experiment/before-state.txt, 25 configs, ROW_Commercial Active) with the restore command written before anything was touched, then 'qmicli -d qrtr://0 --pdc-load-config=<file>' segfaults - rc=139, reproducible, qmicli 1.39.0, specific to that verb since --pdc-list-configs and --pdc-noop both work in the same session, and client-side since the crash follows CTL negotiation. No public bug report for it. ☠️ THE MODEM IS UNCHANGED and needs no restore - verified after the crashes, nothing reached it, so the MBN hypothesis is neither advanced nor damaged. ☠️ It DID settle one previously unverified leg: the Active-generic config reading was from an August capture and still holds today. To unblock: build a different libqmi and retry; the crash is worth an upstream report with a backtrace, which needs a debug build or core dump the device does not currently have
      ...
      after: 167
      prio: 20
      until: a qmicli build whose --pdc-load-config does not segfault
