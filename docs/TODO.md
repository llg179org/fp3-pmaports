...
- [ ] 170. Compare libssc against our SMGR sensor path - transport, API shape and iio-sensor-proxy integration
      lane: phone
      why: The only actively maintained FOSS project in this space, found 2026-09-05 while checking imsd: codeberg.org/DylanVanAssche/libssc, C, ~1.1 MB, meson, src/ tests/ mocking/ docs/, GPLv3, last updated 2026-07-27 - and unlike imsd it HAS CODE. ☠️ NOT a drop-in and do not queue it as one: it targets SSC/SLPI on 'Qualcomm SoCs from 2018 and onwards', while the FP3's SDM632/MSM8953 is 2016-17 and uses the older SMGR. Different sensor framework, different era. What DOES overlap, and is why this is worth an hour: (1) transport - it speaks QMI over QRTR, the same bus our sensor category already needed prerequisites for
