...
- [ ] 177. Package imsd for pmOS (aarch64) and get it to REGISTER - not yet to call. Source: forgejo.catcrafts.net/Catcrafts/imsd, GPL-3.0, Makefile route (clang++ + libc++ + lld + gio-2.0 headers; libc++ 22.1.8 is packaged for Alpine edge aarch64). Then configure it from what this repo already measured: PCSCF from the two P-CSCF addresses in leads/volte-is-provisioned.md (its own P-CSCF discovery from the PDN PCO is NOT implemented, so this value must be given by hand), DEV = the IMS PDN device raised with mmcli --simple-connect apn=ims, LOCAL auto from a global IPv6 - CHECK that the IMS PDN actually gets one, it is listed as auto-detected and may not exist here. Success for this task is a completed SIP REGISTER, nothing more. ☠️ Needs no test call: registration either happens or it does not
      prio: 20 lane: phone
      after: 176
      why: ★ THE IPv4 RISK WAS RAISED AND ANSWERED THE SAME DAY, BEFORE ANY BUILD - by reading the source, which is what this task said to do first. The IMS PDN here is IPv4-only (10.12.230.131/29, zero IPv6 lines though ipv4v6 was requested) and both P-CSCF addresses are IPv4. imsd turns out to be IPv4-capable: every socket in implementations/main.cpp is opened with 'Is6(local) ? AF_INET6 : AF_INET', MakeAddr branches per family, Imsd-Ipsec.cppm picks its prefix length with the same test, and Imsd-Sip.cppm parses both [v6]:port and host:port. The ONLY IPv6 hardcode is DetectLocal(), which runs 'ip -6 addr show dev <dev> scope global' - so LOCAL simply has to be set explicitly, which the README already documents as a variable. ☠️ Read through a summarising fetch, not a clone, and nothing compiled or run; and it says nothing about whether One HU's core accepts SIP and an IPsec SA over v4 from this UE. Remaining unknowns: AMR-WB, and the USIM AKA path on an msm8953-generation modem. captures/2026-09-05_ims-pdn-address/<!-- FP3-QUEUE:END -->
