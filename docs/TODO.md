...
- [@] 63. Reachability test: one call an hour during the day
      ...
      they-do: ★ CHANGED 2026-09-08: these calls now have a purpose they did not have this morning. One HU are measuring this number's radio traffic until 09-10 (see 185) and have ASKED for calls plus the times of any trouble. So make calls during the window - but the thing to record is not the count, it is the WALL-CLOCK TIME and what went wrong. The old caveat still holds for every other purpose: hourly daytime calls are the wrong instrument for the unsampled dangerous corner, which is the FIRST call after a long idle, and more of them do not reach it
...
- [~] 75. Who writes it back: a ModemManager --log-level=DEBUG drop-in
      after: 158
      why: ☠️ READ THIS BEFORE TOUCHING THE DROP-IN: /etc/systemd/system/ModemManager.service.d/zz-fp3-debug.conf must keep BOTH flags. It sets ExecStart, which CLEARS rather than adds, and /etc beats /usr/lib - so a version carrying only --log-level=DEBUG silently removes the --test-quick-suspend-resume that pmOS forces, and the modem then disappears on every resume (#182, measured 2026-09-06/07). The file now carries the reason in a comment. If the debug logging is no longer needed, delete the whole file rather than editing the flags out
      lane: phone
      prio: 30
...
- [ ] 140. B7 + DTS hygiene before fp3-dts is cut: imx363 leftovers (// NOT SURE HOW TO FIND THIS VALUE, //0c40, C++ comments), vdig 1.175 V hardcode -> board l2 constraints, FP3-named delays; audio DTS: drop DMIC4/DMIC5/AMIC5 routes (measured silent 2026-08-02), move slimbam/slim_msm to msm8953.dtsi, interrupts-extended, no output-high in pinctrl; charger DTSI: 48 interrupt-names vs 4 in the binding; XCLR reset polarity ACTIVE_LOW
      why: NOW HAS A MEASURED ARGUMENT, not just ours: Bert Karwatzki's FP3 (2026-09-07) carries the lc898127 actuator and the imx363 at 0x10 instead of 0x1a, and he had to disable our camera node entirely to boot. He also measured that the actuator needs TWO regulators - without vio the i2c transactions time out, because the sensor has already powered down by the time the actuator probes. So the split into a common .dtsi plus per-variant device trees is a hardware fact, not a preference. ★ His lc898217 dual-supply patch is reusable and he published it with his own Signed-off-by, so it can be imported with authorship preserved - see the import rules and what needs his permission in docs/upstreaming/bert-2026-09-07-letter.md
      lane: upstreaming
      after: 178
      prio: 60
...
- [@] 142. Bert's regression: 0314fee3ce35 (msm8953.dtsi system-pc arm,psci-suspend-param 0x41000353 -> 0x42000353, affinity level 2) breaks his hx83112b touchscreen after resume (i2c -110/-6) on a second FP3; reproduce on ours (touch after suspend, before/after revert), and HOLD the msm8953.dtsi idle-state patch out of any series until understood
      why: ANSWERED FROM OUR OWN REPOSITORY 2026-09-07: his tested tip 5bc4d5ebb7c0 is dated 2026-08-30 and carries 1ac2e21fbf3a but NONE of the four touch fixes (cd2745d8d321 + c508e99b9fa9 on 09-04, fb68b1bd764f + 58f135a76e46 on 09-05). So he bisected a tree with no protection on the touch path at all, and the rail fix was written for exactly his -110/-6. ☠️ What is still unexplained is why his REVERT cures it: on this phone the fault needs no suspend (two events 324 s and 47.6 s after a resume with none between; screen-gated 5/5 v 0/5; a clean boot on 0x42000353 with ~513 post-resume touch interrupts and no -110). Arm A ran 2026-09-07 and cannot settle it - the instrument unbinds the driver and the fix IS that driver devm regulator vote. Only arm B is left, and the ask is drafted in /home/fp3/bert-reply-draft.md (outside the repo, by the operator decision)
      the review's section 4 had just listed that dtsi work as sendable
      a regression on a second device outranks that
      lane: phone
      they-do: ARM A IS DONE (2026-09-07, r88 clean boot): 5/5 screen-off, 0/5 screen-on, and its pre-registered criterion turned out unreachable by its own instrument - 142-trigger.sh unbinds the driver first and the rail fix IS that driver devm regulator vote (proven: l6 has 2 consumers bound, 1 unbound). It did give the number #179 could not: the stall fell 15.07 s/-110 to 2.05 s/-5, i.e. the i2c-qup bus-clear recovery. ONLY ARM B IS LEFT and only you can do it: after a deliberate suspend (the phone no longer suspends by itself), MINUTES of tapping with the driver bound - gnome-calculator is enough, >=500 touch interrupts, the 09-04 rate was ~1 x -110 per active minute. ☠️ Say the word and this window arms the suspend. ☠️ It may be impossible until #183 clears: if the screen does not come back after the resume there is nothing to tap - and that itself is the datum
      until: 2026-09-07 19:00
...
- [~] 152. Bert Karwatzki's answer: when it arrives, put his Tested-by on the wcd9335-audio cover + the patches he exercised (integration/7.1.3 @ 5bc4d5ebb7c0), take his module label / EEPROM@0x50 contents into the overlay naming (#144), and fold any wording change he wants on 78a9e301a72f
      ...
      why: HIS ANSWER ARRIVED 2026-09-07: 'Call audio works great!' on his own FP3, integration/7.1.3 @ 5bc4d5ebb7c0, Debian trixie userspace. ☠️ That is NOT yet a Tested-by: submitting-patches.rst requires EXPLICIT permission for it, and 'it works' is a report, not a grant - the next mail must ask. His letter also answers the module question: his FP3 has the lc898127 actuator and the imx363 at 0x10, not 0x1a. Analysis: docs/upstreaming/bert-2026-09-07-letter.md
      prio: 60
...
- [~] 154. Next mail to Bert (person sends): tell him port 52 is DSD on our firmware (measured 2026-08-30), i.e. the qrtr-lookup numbers are per-firmware, which confirms his own caveat; say our qcom-smd-wake keeps the filter in user space and uses a wake IRQ instead of IRQF_NO_SUSPEND; ask whether he wants to be CC'd / co-credited (Reported-by or a Link: to his note) on the series
      lane: upstreaming
      until: when/he-replies
      why: HIS ANSWER ARRIVED 2026-09-07 and it raises the stakes: his point 4 reports 'a lot of unwanted wakeups, a few seconds up to an hour', and in ModemManager work item 694 that blanket IRQF_NO_SUSPEND on the smd irqs gives spurious wakeups every 2 seconds. That is EXACTLY this port's unexplained loose end - rtcwake asked 720 s and got 4 s and 110 s with wakeup_sources naming nothing (2026-09-07). Two independent observers, same platform: it is a property of the platform, not our instrument. So the mail now carries three things: port 52 is DSD on our firmware (his own caveat confirmed), our qcom-smd-wake keeps the filter in user space and uses a wake IRQ rather than IRQF_NO_SUSPEND, and a request for permission to use Tested-by. Analysis: docs/upstreaming/bert-2026-09-07-letter.md
      prio: 60
      they-do: THE LETTER IS DRAFTED, and deliberately NOT in the repo: /home/fp3/bert-reply-draft.md (the operator ruled 2026-09-07 that it is not part of the upstreaming work yet). Read it and send it as your own words. It asks the three permissions (Tested-by on audio for #152, Reported-by on the touchscreen for #142, and whether we may carry his lc898217 patch with his authorship), tells him port 52 is DSD on our firmware, and asks him to test the integration/7.1.3 TIP instead of his revert. ☠️ Do not strengthen it before sending - it deliberately says we cannot explain why his revert helps
...
- [@] 158. Night replication, third attempt: record WHAT WAKES THE AP before running another night — add a per-leg /sys/kernel/debug/wakeup_sources (or wake-reason) snapshot to night-run.sh, then re-run the three-boot replication
      lane: phone
      why: ☠️ HELD until 185 closes (2026-09-10). This run has radio-off windows from 19:00, and One HU are measuring this exact number's radio traffic 09-08 to 09-10. A phone that deliberately stops answering paging during their trace injects FALSE EVIDENCE into their investigation - we would be reporting a fault we caused, into the one channel that can answer what our own instruments cannot. The night is not worth that. Its own three requirements are unchanged and still stand
      until: 09-05 10:00
      they-do: Nothing arms itself: a night run must be armed on the device before 19:00. Say the word and this window arms it. The task's own three requirements stand - record WHAT WAKES THE AP before the run, log each leg's userspace config, and remove the ModemManager debug drop-in. ☠️ That third requirement now has a measured reason and a correction: the drop-in was NOT merely noisy, it removed pmOS's --test-quick-suspend-resume and cost the modem on every resume (#182, 2026-09-07). It has been rewritten to carry both flags, so REMOVING it is now safe and EDITING it is what needs care. ☠️ It conflicts with #179 (closed 2026-09-06) and with #181/#182, which want a suspend plus one call - choose one, do not arm both
...
- [~] 177. Package imsd for pmOS (aarch64) and get it to REGISTER - not yet to call. Source: forgejo.catcrafts.net/Catcrafts/imsd, GPL-3.0, Makefile route (clang++ + libc++ + lld + gio-2.0 headers; libc++ 22.1.8 is packaged for Alpine edge aarch64). Then configure it from what this repo already measured: PCSCF from the two P-CSCF addresses in leads/volte-is-provisioned.md (its own P-CSCF discovery from the PDN PCO is NOT implemented, so this value must be given by hand), DEV = the IMS PDN device raised with mmcli --simple-connect apn=ims, LOCAL auto from a global IPv6 - CHECK that the IMS PDN actually gets one, it is listed as auto-detected and may not exist here. Success for this task is a completed SIP REGISTER, nothing more. ☠️ Needs no test call: registration either happens or it does not
      prio: 20 lane: phone
      after: 176
      why: STATE 2026-09-05 EVENING: imsd is packaged, installed and RUNNING on r82, and reached the protected REGISTER - the core answered 500 Server Internal Error with a Warning: 399 operator diagnostic. Everything before it worked: USIM read over QMI/UIM, 401 challenge, USIM AKA, IPsec SA pair installed (the r82 kernel work in real use), protected TCP connect. ★ And our Contact carries +g.3gpp.icsi-ref=...mmtel, the MMTEL voice tag #163 wondered about. ☠️ The cause of the 500 is NOT known and no candidate is evidence-backed: the IMSI-derived temporary IMPU was written up as the leading suspect and then demoted the same hour, because main.cpp line 987 shows imsd uses it deliberately and correctly for a USIM-only card, which is what 3GPP specifies. The method that would settle it is the one upstream built DumpRaw for - diff our REGISTER header by header against the stock stack's, and this project HAS that oracle: UT registers successfully against the same core with the same card (#163). That needs a UT-side capture, the wall #54 and #64 stand at. captures/2026-09-05_imsd-first-register/
      until: a reply from One HU. The letter went to ugyfelszolgalat@one.hu on 2026-09-06 evening. It asks (a) what the operator diagnostic 'Warning: 399 5144.2233.S.260.5.94.255.255.5938.0.0' means - the one question only they can answer; (b) whether MMTEL, not merely IMS registration, is authorised for this subscription, since the STOCK stack registers on this device with this card and still takes every call over CSFB; (c) which element of our REGISTER is refused. It carries the identifiers that let them find the transaction: Call-ID qi7755d7dy9lv9db, Via branch z9hG4bK512v6kj5b4dsfzuz, CSeq 2 REGISTER, sent 2026-09-06 22:03:58.629 CEST, the 500 back at 22:03:58.841. Their stated complaint deadline is 30 days, so chase after 2026-10-06. Fallback if no useful answer: the oracle diff, the same capture wall as #54 and #64
- [@] 178. measure the himax retry half of #157 in ordinary use on r83: errors per hour and per 1000 touch interrupts, against the r82 rate of 5 x (-110/-6) pairs + one -5 wedge in 11 min --why ☠️ The reproducer CANNOT see this half - it probes a raw i2c address, not the driver's event read. And with the retry in place a transient failure now produces NO log line at all, so 'zero errors' is ambiguous between 'no faults' and 'faults absorbed'; distinguishing them needs a counter the driver does not have. The comparable r82 numbers are in docs/power/bringup/captures/2026-09-05_157-fault-rate-on-r82/. A clean run only means something if the panel was actually used - check 59 says <500 touch interrupts measures nobody touching it --lane phone
      when: the operator has used the phone for at least 36 ACTIVE minutes - sampling intervals in which the touch interrupt counter moved, NOT wall time and NOT interrupt count. ☠️ Names no release or filename on purpose; ask the device (see #179's note). The r83 window is closed and measured NOTHING: 104 min wall, 2002 interrupts, 1590 of them in one two-minute burst, 3 active minutes. Analyse with touch-exposure.py, which groups by boot_id before differencing - /proc/interrupts resets at boot and the first attempt reported -2670 interrupts
      after: 179
      until: 2026-09-07 19:00

- [@] 181. Let the phone actually suspend when idle (logind IdleAction), leave it overnight, then take the FIRST incoming call after that long idle - the unsampled dangerous corner of #63 --why ☠️ #63 says in its own note that the dangerous corner is UNSAMPLED and that hourly calls are the wrong instrument for it, not too few of it: they never test a phone that has been idle for hours. Measured 2026-09-06 05:50 why the corner cannot be reached by simply waiting: after 5 h 03 with the screen off, /sys/power/suspend_stats/success = 0 - the phone had not suspended ONCE. logind's IdleAction is unset in logind.conf and logind.conf.d, so the default 'ignore' applies and nothing ever puts it down; systemd-inhibit shows only delay-mode sleep inhibitors (ModemManager, NetworkManager, rtkit, UPower, gsd) and no block-mode one. This matches what docs/power/ already records - pmOS never suspends by itself here - while s2idle itself works when triggered (6/6 suspends, full duration). So the corner needs a CONFIGURATION CHANGE, not patience. Procedure: (1) record the before-state (logind.conf + any drop-in, systemd-inhibit --list, suspend_stats) into the capture BEFORE changing anything - it is the way back; (2) set IdleAction=suspend with an IdleActionSec, as a drop-in so the change is one file to delete; (3) confirm it actually suspends (suspend_stats/success climbs, and the phone stops answering ssh); (4) leave it overnight UNTOUCHED; (5) the operator places ONE incoming call in the morning and reports whether it rang and how long it took. ☠️ DO NOT SSH IN BEFORE THE CALL - a login wakes the phone and destroys the very state being sampled; docs/power/ records a replication night lost exactly that way, which is why fp3-ssh carries the fp3-measure lock. ☠️ It conflicts with #158 (night replication): both want the night, so choose one. ☠️ Ordered AFTER the touch work by the operator 2026-09-06: an unattended night is worth little while a -EIO storm can still kill the touch IRQ --lane phone
      after: 183
      they-do: NOTHING for now - #181 is no longer blocked by #182 (fixed and verified 2026-09-07 08:24: 26 min 12 s suspend, ended by an incoming call, and it RANG). It is now blocked by #183: the drop-in this task needs is exactly the one that was removed, and with it back the phone falls into the phoc DSI resume bug unattended - the morning sample would be a phone whose screen never comes back, read as a reachability result. When #183 clears, re-arm and the ask is one incoming call in the morning
      when: the morning after an untouched night
- [~] 183. phoc cannot restore DSI-1 on resume: 6 of 6 resumes followed by a screen-on attempt carry 'Swapchain for output DSI-1 failed test' + 'Failed to commit power mode change to 1' in the SAME SECOND, while DRM reports dpms=On status=connected and the backlight lit. ScreenSaver.SetActive then moves it in neither direction, and phosh holds a handle-power-key BLOCK inhibitor so the power button does nothing. greetd restart does not recover it (fresh phoc gets 'Atomic commit failed: Resource busy'); a reboot does. ☠️ This was mistaken for #182 for two days - #182 is now fixed and verified, the phone rang, and the screen still did not come back. NEXT STEP IS A WEB SEARCH, NOT CODE: check phoc/wlroots versions against upstream reports before writing anything. Capture: docs/power/bringup/captures/2026-09-07_phoc-cannot-restore-dsi-on-resume/ --why it makes the phone unusable after every suspend, and it silently double-counted as evidence for a modem fault --prio 20
      until: when pmOS edge resolves gnome-settings-daemon-mobile vs postmarketos-ui-phosh-33 - until then apk upgrade -a, apk add phoc and apk add phosh all fail identically and phoc-0.57.0-r0 is unreachable. SEARCH DONE 2026-09-07: pmaports#3062 is the same symptom class on a Samsung A5, open since 2024, no fix; no match for the exact phoc lines. ☠️ Do NOT force it - rootfs is at 90 percent (safety item 9). Mitigated by accident meanwhile: no idle suspend since 08:45, so the fault is unreachable unattended - which is exactly what #181 needs back
- [x] 184. Himax IDLE MODE: test whether the controller's idle scan is the mechanism behind the same-side tap outages. Write 0x17 to register 0x10007088 byte 0 (0x1F = idle on, 0x17 = off), named by the oracle's vendor driver himax_idle_mode() in ubports-fp3-kernel/drivers/input/touchscreen/hxchipset83112b/himax_ic.c. Needs the vendor register protocol (himax_register_read / himax_flash_write_burst), NOT a plain i2c write
      when: 2026-09-10 19:10: FIX ON THE DEVICE, awaiting the operator's tapping run. Idle mode is now cleared by the driver (0x10007088 byte0 0x3f -> 0x37, verified +1 s and +5 s after probe), charger mode set (0x10007f38 = a55aa55a). Three commits on wip/7.1.3/touch, hot-swapped module; package build follows. Test: tap with the same 3 s pauses; read taptest BREAK explained=NO + kernel-contacts + hx_irq kprobe. Effect on the fault NOT measured yet - lost-taps.md carries the trail
      they-do: Tap as usual with the same pauses - a few seconds of not touching, then resume. That pause is the trigger; everything else is already excluded. Nothing needs judging about whether a tap happened: the high tone marks each unexplained break and the logs carry the rest
      why: CLOSED 2026-09-11 00:53, REFUTED by a within-session 2x2 (tools/hx-legs.py): idle off/chg on 1.17 pct, off/off 3.00, on/off 0.60, on/on (shipped) 0.66 pct unexplained breaks over 800-1053 taps each; contacts >=100 ms flat at 0.8-1.7 pct. Neither firmware word is the mechanism. The driver commits (idle off, charger mode) are correct code with no measured benefit - keep or drop is the operator's call. Next discriminator: the same tap run on the UT oracle with an evdev logger
- [x] 185. One HU radio-side traffic measurement, 2026-09-08 to 2026-09-10: use the service, log every call with its timestamp and outcome, send the notes to them within 72 h of the window closing (so by 2026-09-13)
      lane: phone prio: 5 when: the window is OPEN NOW and closes 2026-09-10
      the report is due by 2026-09-13 why: One HU answered the 2026-09-06 letter (see 177): they are running a radio-side check on the dev card's number and the device that holds it, 09-08 to 09-10, and ask for the operator's own record of when problems occurred so they can correlate it against their measurements. This is the only route left that can answer the question our own instruments cannot - the operator diagnostic 'Warning: 399 ... 5938.0.0' behind the 500 on our SIP REGISTER, and whether MMTEL rather than mere IMS registration is authorised for this subscription. The capture wall of 54 and 64 does not have to be climbed if the operator simply tells us. It is also time-boxed and cannot be repeated on demand they-do: Place and take calls normally across the two days, and note the WALL-CLOCK TIME of anything that goes wrong: a call that does not connect, one that does not ring, bad or one-way audio, a drop, a long setup. Their side is useless without your times. Approximate is fine, a minute is enough resolution for a radio trace
      why: DONE 2026-09-10: the letter went out through One HU's web form (CCT-092026-213060, BT-092026-212446) with the PDF - one focused event, the 20:12:58 REGISTER 500 inside their window, three Warning:399 occurrences (5.94/5.3/5.75), transaction ids. Raw material in /mnt/1TB/pmos/fp3-raw-logs/2026-09-08_imsd-sip/. The sent text says '38 of 38 since 09-03'; the verified figure is 39 of 39 since 09-02 - correct it in any follow-up. Waiting on their reply is 177
- [ ] 186. Automated tap stimulus for the lost-taps fault: replace hand tapping with a repeatable, timed two-point stimulus so a configuration A/B is a number and not a rhythm. The 2026-09-11 2x2 (docs/touch/lost-taps.md) needed four ~1000-tap hand legs and one 123-tap leg still read 8.9 pct against 0.66 pct on its repeat
      lane: phone why: Operator 2026-09-11: 'ha megmarad a touch tévesztés, további automatizált tesztelésre lesz szükség'. The rate is 0.6-3 pct of taps and operator-dependent
      only an automated stimulus with fixed cadence and pauses makes legs comparable. Candidates in lost-taps.md 'automating the stimulus'. After it: the same run on the UT oracle (54/64)
<!-- FP3-QUEUE:END -->
