...
| [`volte/`](volte/README.md) | **4G voice**: why the phone still rings over CS fallback, the ten-link chain from "does the network provision IMS" to "does a call run over IMS" with the state of each link measured, the two possible paths and why one is measured against, how to reproduce today's state, and the traps this area has already sprung |
| [`touch/`](touch/142-i2c-stall.md) | why the hx83112b panel freezes for exactly 15 seconds: the QUP timeout constant that sets the duration, the three measured gates that trigger it, everything eliminated by measurement, and the three things the vendor kernel does that we do not |
