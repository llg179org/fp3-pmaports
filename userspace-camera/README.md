...
| `snapshot/0012`…`0020` | the incremental fixes between the two reworks: refocus before a full-resolution capture, a *Reset to Defaults* button, re-applying the focus mode once frames are flowing, restoring it after a photo, dropping the *Recalibrate view size* row, two `videocrop` ordering fixes, keeping a hand-set focus across reconfigures, and the face focus mode. Listed together because each is a few lines; read the files for the detail |
| [`snapshot/0021-camera-one-focus-mode-control.patch`](snapshot/0021-camera-one-focus-mode-control.patch) | **one focus control instead of two settings and a hidden slider.** An `Autofocus` switch and a `Focus Mode` dropdown could contradict each other, the dropdown's `manual` value was wired to nothing at all, and the lens-distance slider was reachable only behind a switch in the *preferences*. Replaced by a single `focus-mode` — `continuous` / `face` / `tap` / `off` — in a popover on the camera page next to the flash, with the dioptre slider, live in `off`. A tap takes the picture in `tap` mode, and the focus-before-capture decision moves into `take_picture()` so a tap and the shutter button cannot drift apart. The two focus waits were **measured** rather than assumed: 900 ms against a ~4.8 s sweep, so the shutter fired mid-scan. Focusing on demand also asks for `AfMeteringWindows`; aiming that at the tapped point is impossible from here, because PipeWire's libcamera plugin maps only scalar controls and `AfWindows` is a rectangle array. The design it is built to is [`snapshot/FOCUS-UI-SPEC.md`](snapshot/FOCUS-UI-SPEC.md) |
| [`snapshot/0022-camera-only-a-tap-on-the-picture-takes-a-picture.patch`](snapshot/0022-camera-only-a-tap-on-the-picture-takes-a-picture.patch) | **a tap on a control is not a tap on the picture.** The controls sit in a layer spanning the whole page and the tap gesture is on the page rather than on the viewfinder - it has to be, or the overlay would swallow every touch - so in `tap` mode a press on the menu, on the shutter bar or on the gallery thumbnail fired the shutter as well. The thumbnail was the worst of the three: it saved a picture of wherever the phone was pointing while the finger left the camera. The gesture now asks what the tap landed on, walking up from the picked widget, and acts only when the answer is the picture and the page showing is the camera |
| [`snapshot/0023-camera-make-the-focus-mode-and-its-slider-arrive.patch`](snapshot/0023-camera-make-the-focus-mode-and-its-slider-arrive.patch) | **the chosen focus mode was not reaching the camera, and the distance slider was never built.** One cause: the mode is read from the setting while the window is built, long before PipeWire has a node whose controls can be written, and a write with nowhere to go reports nothing - so the camera stayed in its own default, continuous hunting, while the menu showed the mode the user picked. Nothing can be read back to check, so the mode is now re-sent a few times a second until the controls exist, and the failure to ever find them is logged. The slider is built from the lens's own bounds and the popover was built once at start-up, with no camera to ask: it was missing for good, including in `off` where it is the only thing that moves the lens. It is now built when the camera comes up, and only while still missing, so a later reconfigure cannot shut a popover the user has open |
