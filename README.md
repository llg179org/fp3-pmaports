...
| `power` | the platform's own low-power plumbing and the instruments that measure it: the RPM sleep-stats region, the per-master sleep records, the PSCI affinity level the APSS handshake needs and the MPM notification path ([`docs/power/`](docs/power/README.md)) | partly — the description patches are upstream material and the genpd `bool` fix already is upstream's. Two candidates found by measurement and not yet written: `msm8916-wcd-digital.c` taking its LPASS clocks at probe instead of under runtime PM, and `qcom_smd-regulator.c` never writing the sleep set |
